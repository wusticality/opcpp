...
class opNodeBase {
   ...
   public:
    ...
    // declare this in subclasses - the default constructor will always call
    // this
    void Init() {}
    ...
    opNode* GetParent() { return parent; }

    int GetLine() { return line; }

    const iterator& GetPosition() { return pos; }

    iterator GetBegin() { return children.Begin(); }

    iterator GetLast() { return children.Last(); }

    iterator GetEnd() { return children.End(); }

    // don't use if possible, instead use HasNumChildren
    int NumChildren() { return (int)children.Size(); }

    bool HasNumChildren(int num) { return children.HasSize(num); }

    FileNode* GetFile() { return file; }

    /**** set ****/

    void SetId(Token _id) { id = _id; }

    void SetParent(opNode* _parent) { parent = _parent; }

    void SetLine(int _line) { line = _line; }

    void SetFile(FileNode* infile) { file = infile; }
    ...
    // use sparingly
    void SetPosition(iterator newpos) { pos = newpos; }

    void IncrementPosition() { ++pos; }

    void DecrementPosition() { --pos; }

    bool IsEmpty() { return !HasNumChildren(1); }
    ...
    template <class N>
    void CollapseNode(stacked<N>& collapse, iterator index);
    template <class N>
    void CollapseNodeAtCurrent(stacked<N>& collapse);
    template <class N>
    void AppendNode(stacked<N>& newchild);
    template <class N>
    void PrependNode(stacked<N>& newchild);
    template <class N>
    void InsertNode(stacked<N>& newChild, iterator index);
    template <class N>
    void InsertNodeAtCurrent(stacked<N>& newChild);
    template <class N>
    stacked<N> PopNode(N* node);
...
class opNode : public opNodeBase {
   public:
    ...
    ~opNode() { UnRegisterNode(); }

    void Init() {}

    void PostInit() {
        // for checking that ErrorName is defined
        if (opParameters::Get().DeveloperMode) ErrorName();
    }

/**** memory tracking support ****/

#ifdef _DEBUG
    void SetAllocationLocation(const opString& s) { AllocationLocation = s; }

    const opString& GetAllocationLocation() { return AllocationLocation; }

    int GetAllocationNumber() { return AllocationNumber; }

    static int GetStaticAllocationNumber() { return StaticAllocationCounter; }
   ...
   protected:
    ...
    void UnRegisterNode() { opMemoryTracker::UnRegister(this); }
   ...
   public:
    ...
    virtual void visit(opVisitor& visitor) {
        ...
        try {
            if (visitor.enter(this)) visit_children(visitor);
    ...
    virtual opString GetTreeValue() { return ""; }
    ...
    virtual opString GetNodeTypeSource() { return ""; }

    /**** printing ****/

    // Print Node (this node and all children)
    // should usually just call PrintOriginal, PrintTransformed,
    // PrintNodeChildren  must define for all classes
    ...
    virtual void PrintXml(opXmlStream& stream) { PrintXmlChildren(stream); }
   ...
   public:
    ...
    virtual bool IsGrammar() { return true; }

    virtual bool Parse() { return true; }

    virtual bool Verify() { return false; }

    virtual FileNode* ToFileNode() { return NULL; }
    ...
    void UndefinedFunctionError(const opString& functionsignature) {
        ...
        Log(classlocation + " : error : " + functionsignature +
            " not defined for class " + classtype);
    }
    ...
    bool IsCurrent(Token compare) {
        if (GetPosition() == GetEnd()) return false;
    ...
    bool IsPrevious(Token compare) {
        if (!IsEmpty()) {
            if (GetPosition() == GetBegin()) return false;
    ...
    virtual PreprocessorStatementNode* ToPreprocessorStatementNode() {
        return NULL;
    }
   ...
   public:
    ...
    bool IgnoreUntil();
    template <Token t1, Token t2, Token t3, Token t4, Token t5, Token t6,
              Token t7, Token t8, Token t9>
    bool IgnoreUntil();
    template <Token t1, Token t2, Token t3, Token t4, Token t5, Token t6,
              Token t7, Token t8, Token t9, Token t10>
    bool IgnoreUntil();
    template <Token t1, Token t2, Token t3, Token t4, Token t5, Token t6,
    ...
    void Disallow(Token t) { DisallowBase(t, false); }

    // disallow - recursive version
    void DisallowRecursive(Token t) { DisallowBase(t, true); }

    // PushUntil range version - pushes in the range [position,end)
    template <class T>
    stacked<T> PushUntil(iterator end);

    // Push Until
    // Adds nodes to an existing node until one of the tokens in t is reached.
    // Throws an error when EOF or EOB is reached, if so desired.

    // pushuntil for 1 token
    void PushUntilAdd(opNode* addto, Token t, bool bErrorOnMissing = true);

    // pushuntilor - nongeneration versions
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        Token t10, bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        Token t10, Token t11, bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        Token t10, Token t11, Token t12,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        Token t10, Token t11, Token t12, Token t13,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        Token t10, Token t11, Token t12, Token t13, Token t14,
                        bool bErrorOnMissing = true);
    void PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3, Token t4,
                        Token t5, Token t6, Token t7, Token t8, Token t9,
                        Token t10, Token t11, Token t12, Token t13, Token t14,
                        Token t15, bool bErrorOnMissing = true);

    // pushuntil for 1 token - generation version
    template <class T>
    stacked<T> PushUntil(Token t, bool bErrorOnMissing = true);

    // pushuntilor - generation versions
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13, Token t14,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
    ...
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13, Token t14,
                           Token t15, Token t16, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13, Token t14,
                           Token t15, Token t16, Token t17,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13, Token t14,
                           Token t15, Token t16, Token t17, Token t18,
                           bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13, Token t14,
                           Token t15, Token t16, Token t17, Token t18,
                           Token t19, bool bErrorOnMissing = true);
    template <class T>
    stacked<T> PushUntilOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                           Token t6, Token t7, Token t8, Token t9, Token t10,
                           Token t11, Token t12, Token t13, Token t14,
                           Token t15, Token t16, Token t17, Token t18,
                           Token t19, Token t20, bool bErrorOnMissing = true);

    // push until end
    template <class T>
    stacked<T> PushUntilEnd();
    ...
    template <class T>
    inline stacked<T> Expect(Token t) {
        // if its a grammar
        // compile time assert: verify this is true
        // TODO: test this
        // assert(!tokenmapping::NodeTypeIsGrammar<T>() ||
        // tokenmapping::NodeTypeToken<T>::id == t);
        vector<Token> tokens;
        tokens.push_back(t);
    ...
    template <class T>
    stacked<T> ExpectOr(Token t1, Token t2);
    template <class T>
    stacked<T> ExpectOr(Token t1, Token t2, Token t3);
    template <class T>
    stacked<T> ExpectOr(Token t1, Token t2, Token t3, Token t4);
    template <class T>
    stacked<T> ExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5);
    template <class T>
    stacked<T> ExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                        Token t6);
    template <class T>
    ...
    stacked<T> ExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                        Token t6, Token t7, Token t8, Token t9, Token t10,
                        Token t11, Token t12, Token t13, Token t14, Token t15);
    ...
    // reverse expect
    template <class T>
    inline stacked<T> ReverseExpect(Token t, opNode* after = NULL) {
        vector<Token> tokens;
        tokens.push_back(t);
    ...
    // reverse expect or
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3,
                               opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                               opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, Token t8,
                               opNode* after = NULL);
    ...
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, Token t8, Token t9,
                               Token t10, opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, Token t8, Token t9,
                               Token t10, Token t11, opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, Token t8, Token t9,
                               Token t10, Token t11, Token t12,
    ...
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, Token t8, Token t9,
                               Token t10, Token t11, Token t12, Token t13,
                               Token t14, opNode* after = NULL);
    template <class T>
    stacked<T> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4, Token t5,
                               Token t6, Token t7, Token t8, Token t9,
                               Token t10, Token t11, Token t12, Token t13,
                               Token t14, Token t15, opNode* after = NULL);
    ...
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3,
                                    opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7,
                                    opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
    ...
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, Token t10, opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, Token t10, Token t11,
                                    opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, Token t10, Token t11, Token t12,
                                    opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, Token t10, Token t11, Token t12,
                                    Token t13, opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, Token t10, Token t11, Token t12,
                                    Token t13, Token t14, opNode* after = NULL);
    stacked<opNode> ReverseExpectOr(Token t1, Token t2, Token t3, Token t4,
                                    Token t5, Token t6, Token t7, Token t8,
                                    Token t9, Token t10, Token t11, Token t12,
                                    Token t13, Token t14, Token t15,
    ...
    template <class T>
    T* CheckOr(Token t1, Token t2);
    template <class T>
    T* CheckOr(Token t1, Token t2, Token t3);
    template <class T>
    T* CheckOr(Token t1, Token t2, Token t3, Token t4);
    template <class T>
    T* CheckOr(Token t1, Token t2, Token t3, Token t4, Token t5);
    template <class T>
    T* CheckOr(Token t1, Token t2, Token t3, Token t4, Token t5, Token t6);
    template <class T>
    T* CheckOr(Token t1, Token t2, Token t3, Token t4, Token t5, Token t6,
               Token t7);
    template <class T>
    ...
    opNode* CheckOr(Token t1, Token t2, Token t3, Token t4, Token t5, Token t6,
                    Token t7, Token t8, Token t9, Token t10, Token t11,
                    Token t12, Token t13, Token t14);
    opNode* CheckOr(Token t1, Token t2, Token t3, Token t4, Token t5, Token t6,
                    Token t7, Token t8, Token t9, Token t10, Token t11,
                    Token t12, Token t13, Token t14, Token t15);
    ...
    TerminalNode* AppendTerminalNode(const opString& stringvalue,
                                     Token tokenid);
    TerminalNode* PrependTerminalNode(const opString& stringvalue,
                                      Token tokenid);
   ...
   private:
    ...
    stacked<T> ExpectBase(const vector<Token>& Tokens, bool bReversed = false,
                          opNode* before = NULL);

    template <class T>
    stacked<T> PushUntilEndBase();

    opNode* PushUntilBase(opNode* addto, const vector<Token>& t,
                          bool bErrorOnMissing);
};
...
inline T* fast_node_cast(opNode* node) {
    if (node->GetId() == tokenmapping::NodeTypeToken<T>::id) return (T*)node;
    return NULL;
}

template <>
inline TerminalNode* fast_node_cast<TerminalNode>(opNode* node) {
    if (TokenFunctions::IsTerminal(node->GetId())) return (TerminalNode*)node;
    return NULL;
}
