...
class ListUtility : public Parent {
   public:
    ...
    void MakeDelimiterList(vector<ArgumentType*>& container) {
        // NOTE: only want to build this list once, if its non-empty assume it
        // was built.
        ...
        if (!this->IsEmpty()) {
            while (1) {
                stacked<ArgumentType> arg =
                    opNode::PushUntil<ArgumentType>(Delimiter, false);
            ...
            for (int i = 0; i < size; i++) {
                stacked<opNode> tempstacked =
                    stacked<opNode>::buildstacked(container[i]);
...
class UnInlineSupport : public Parent {
    ...
    bool GetInlineMode() {
        ...
        if (!binline && !buninline) doinline = opParameters::Get().InlineAll;
