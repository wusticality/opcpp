...
inline bool Categories<Parent>::Parse() {
    PARSE_START;
    { FindCategories(); }
    PARSE_END;
}
...
inline void Categories<Parent>::FindCategories() {
    LOOP_START(G_CATEGORY) {
        HIT(T_CATEGORY) {
            stackedcontext<CategoryNode> newNode =
                opNode::Make<CategoryNode>(T_CATEGORY);
            ...
            stacked<CategoryBodyNode> body =
                opNode::Transform<CategoryBodyNode>(bbn);
...
template <class Parent>
inline void CodeLocations<Parent>::FindCodeLocations() {
    LOOP_START(G_CODE) {
        HIT(T_CODE) {
...
template <class Parent>
inline bool CategoryLocations<Parent>::Parse() {
    PARSE_START;
...
inline void CategoryLocations<Parent>::FindCategoryLocations() {
    LOOP_START(G_CATEGORY_LOCATION) {
        HIT(T_LOCATION) {
            stackedcontext<CategoryLocationNode> newNode =
                opNode::Make<CategoryLocationNode>(T_LOCATION);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<CategoryLocationBodyNode> body =
                opNode::Transform<CategoryLocationBodyNode>(bbn);
...
template <class Parent>
inline bool Disallows<Parent>::Parse() {
    PARSE_START;
    { FindDisallows(); }
    PARSE_END;
}
...
inline void Disallows<Parent>::FindDisallows() {
    LOOP_START(G_DISALLOW) {
        HIT(T_DISALLOW) {
            stackedcontext<DisallowNode> newNode =
                opNode::Make<DisallowNode>(T_DISALLOW);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<DisallowBodyNode> body =
                opNode::Transform<DisallowBodyNode>(bbn);
...
template <class Parent>
inline bool CategoryMaps<Parent>::Parse() {
    PARSE_START;
    { FindCategoryMaps(); }
    PARSE_END;
}
...
inline void CategoryMaps<Parent>::FindCategoryMaps() {
    ...
    FindCategoryMap<G_CATEGORY_FUNCTIONMAP, T_FUNCTIONMAP,
    ...
    LOOP_START(grammar) {
        HIT(token) {
            ...
            if (this->IsCurrent(G_BRACE_BLOCK)) {
                stacked<BraceBlockNode> bbn =
                    opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
                stacked<CategoryMapBodyNode> body =
                    opNode::Transform<CategoryMapBodyNode>(bbn);
...
template <class Parent>
inline bool DataModifiers<Parent>::Parse() {
    PARSE_START;
    { FindDataModifiers(); }
    PARSE_END;
}
...
inline void DataModifiers<Parent>::FindDataModifiers() {
    LOOP_START(G_DATAMODIFIER) {
        HIT(T_DATAMODIFIER) {
            stackedcontext<DataModifierNode> newNode =
                opNode::Make<DataModifierNode>(T_DATAMODIFIER);
            ...
            if (this->IsCurrent(G_PAREN_BLOCK)) {
                ...
                stacked<ModifierArgumentNode> args =
                    opNode::Transform<ModifierArgumentNode>(paren);
...
template <class Parent>
inline bool FunctionModifiers<Parent>::Parse() {
    PARSE_START;
...
inline void FunctionModifiers<Parent>::FindFunctionModifiers() {
    LOOP_START(G_FUNCTIONMODIFIER) {
        HIT(T_FUNCTIONMODIFIER) {
            stackedcontext<FunctionModifierNode> newNode =
                opNode::Make<FunctionModifierNode>(T_FUNCTIONMODIFIER);
...
template <class Parent>
inline bool CriteriaExpressions<Parent>::Parse() {
    PARSE_START;
...
inline void CriteriaExpressions<Parent>::FindIs() {
    LOOP_START(grammar) {
        HIT(token) {
            ...
            stacked<NodeBodyClass> body =
                opNode::PushUntil<NodeBodyClass>(T_SEMICOLON);
...
template <class Parent>
inline bool Notes<Parent>::Parse() {
    PARSE_START;
    { FindNotes(); }
    PARSE_END;
}
    ...
    LOOP_START(G_NOTE) {
        HIT(T_NOTE) {
            ...
            if (this->IsCurrent(G_BRACE_BLOCK)) {
                stacked<BraceBlockNode> bbn =
                    opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
                stacked<NoteBodyNode> body =
                    opNode::Transform<NoteBodyNode>(bbn);
...
template <class Parent>
inline bool NoteDefinitions<Parent>::Parse() {
    PARSE_START;
    { FindNoteDefinitions(); }
    PARSE_END;
}
...
inline void NoteDefinitions<Parent>::FindNoteDefinitions() {
    LOOP_START(G_NOTE_DEFINITION) {
        HIT(T_NOTE) {
            stackedcontext<NoteDefinitionNode> newNode =
                opNode::Make<NoteDefinitionNode>(T_NOTE);
            ...
            stacked<ParenBlockNode> ppn =
                opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
            stacked<NoteArgumentListNode> args =
                opNode::Transform<NoteArgumentListNode>(ppn);
...
template <class Parent>
inline bool Enumerations<Parent>::Parse() {
    PARSE_START;
    { FindEnumerations(); }
    PARSE_END;
}
...
inline void Enumerations<Parent>::FindEnumerations() {
    LOOP_START(G_ENUMERATION) {
        HIT(T_ENUMERATION) {
            stackedcontext<EnumerationNode> newNode =
                opNode::Make<EnumerationNode>(T_ENUMERATION);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<EnumerationBodyNode> body =
                opNode::Transform<EnumerationBodyNode>(bbn);
...
template <class Parent>
inline bool EnumerationLocations<Parent>::Parse() {
    PARSE_START;
...
inline void EnumerationLocations<Parent>::FindEnumerationLocations() {
    LOOP_START(G_ENUMERATION_LOCATION) {
        HIT(T_LOCATION) {
            stackedcontext<EnumerationLocationNode> newNode =
                opNode::Make<EnumerationLocationNode>(T_LOCATION);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<EnumerationLocationBodyNode> body =
                opNode::Transform<EnumerationLocationBodyNode>(bbn);
...
template <class Parent>
inline bool EnumerationMaps<Parent>::Parse() {
    PARSE_START;
    { FindEnumerationMaps(); }
    PARSE_END;
}
...
inline void EnumerationMaps<Parent>::FindEnumerationMaps() {
    LOOP_START(G_ENUMERATION_MAP) {
        HIT(T_ENUMMAP) {
            stackedcontext<EnumerationMapNode> newNode =
                opNode::Make<EnumerationMapNode>(T_ENUMMAP);
    ...
    LOOP_START(grammar) {
        HIT(T_ID) {
            ...
            if (t->GetValue() == ModifierClass::StaticModifierName()) {
                stackedcontext<ModifierClass> newNode =
                    opNode::Make<ModifierClass>(T_ID);
...
template <class Parent>
inline bool CriteriaValueModifiers<Parent>::Parse() {
    PARSE_START;
    ...
    LOOP_START(G_CRITERIA_VALUE_MODIFIER) {
        HIT(G_PAREN_BLOCK) {
            if (this->IsPrevious(T_ID)) {
                stackedcontext<CriteriaValueModifierNode> newNode =
                    opNode::Make<CriteriaValueModifierNode>(G_PAREN_BLOCK);

                stacked<ParenBlockNode> paren =
                    opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
                stacked<CriteriaArgumentNode> arg =
                    opNode::Transform<CriteriaArgumentNode>(paren);

                newNode->SetArgument(*arg);

                stacked<TerminalNode> id =
                    opNode::ReverseExpect<TerminalNode>(T_ID, *arg);
...
template <class Parent>
inline bool CriteriaGroups<Parent>::Parse() {
    PARSE_START;
    { FindCriteriaGroups(); }
    PARSE_END;
}
...
inline void CriteriaGroups<Parent>::FindCriteriaGroups() {
    LOOP_START(G_CRITERIA_GROUP) {
        HIT(G_PAREN_BLOCK) {
            stacked<ParenBlockNode> paren =
                opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
            stacked<CriteriaGroupNode> newNode =
                opNode::Transform<CriteriaGroupNode>(paren);
            this->InsertNodeAtCurrent(newNode);
        }
    }
    LOOP_END;
...
template <class Parent>
inline bool Extensions<Parent>::Parse() {
    PARSE_START;
    { FindExtensions(); }
    PARSE_END;
}
...
inline void Extensions<Parent>::FindExtensions() {
    LOOP_START(G_EXTENSION) {
        HIT(T_EXTENSION) {
            stackedcontext<ExtensionNode> newNode =
                opNode::Make<ExtensionNode>(T_EXTENSION);
...
template <class Parent>
inline bool ExtendPoints<Parent>::Parse() {
    PARSE_START;
    { FindExtendPoints(); }
    PARSE_END;
}
...
inline void ExtendPoints<Parent>::FindExtendPoints() {
    LOOP_START(G_EXTEND_POINT) {
        HIT(T_EXTEND_POINT) {
            stackedcontext<ExtendPointNode> newNode =
                opNode::Make<ExtendPointNode>(T_EXTEND_POINT);
...
template <class Parent>
inline bool ExtensionPoints<Parent>::Parse() {
    PARSE_START;
    { FindExtensionPoints(); }
    PARSE_END;
}
...
inline void ExtensionPoints<Parent>::FindExtensionPoints() {
    LOOP_START(G_EXTENSION_POINT) {
        HIT(T_EXTENSION_POINT) {
            stackedcontext<ExtensionPointNode> newNode =
                opNode::Make<ExtensionPointNode>(T_EXTENSION_POINT);
...
template <class Parent>
inline bool FileDeclarationLocations<Parent>::Parse() {
    PARSE_START;
...
inline void FileDeclarationLocations<Parent>::FindFileDeclarationLocations() {
    LOOP_START(G_FILE_DECLARATION_LOCATION) {
        HIT(T_LOCATION) {
            stackedcontext<FileDeclarationLocationNode> newNode =
                opNode::Make<FileDeclarationLocationNode>(T_LOCATION);
...
template <class Parent>
inline bool FileDeclarations<Parent>::Parse() {
    PARSE_START;
...
inline void FileDeclarations<Parent>::FindFileDeclarations() {
    LOOP_START(G_FILE_DECLARATION) {
        HIT(T_FILE_DECLARATION) {
            stackedcontext<FileDeclarationNode> newNode =
                opNode::Make<FileDeclarationNode>(T_FILE_DECLARATION);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<FileDeclarationBodyNode> body =
                opNode::Transform<FileDeclarationBodyNode>(bbn);
...
template <class Parent>
inline bool DialectNamespaces<Parent>::Parse() {
    PARSE_START;
...
inline void DialectNamespaces<Parent>::FindDialectNamespaces() {
    LOOP_START(G_DIALECT_NAMESPACE) {
        HIT(T_NAMESPACE) {
            stackedcontext<DialectNamespaceNode> newNode =
                opNode::Make<DialectNamespaceNode>(T_NAMESPACE);
