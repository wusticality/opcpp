...
class opStringStream {
   public:
    ...
    void TrimLineEnd() { linestream = linestream.TrimRight(); }

    /**** operators ****/

    inline friend opStringStream& operator<<(opStringStream& oin,
                                             const endlinestruct& endline) {
    ...
    void SetHeadingString(const opString& s) { headingstring = s; }

    void SetFooterString(const opString& s) { footerstring = s; }
    ...
    void PrintComment(const opString& s) {
        ...
        if (opParameters::Get().Compact && !ForceComments) {
            return;
        }
    ...
    static void SetForceComments(bool inbool) { ForceComments = inbool; }

    static bool GetForceComments() { return ForceComments; }
...
class opSectionStream {
   public:
    ...
    void Indent() { body.Indent(); }

    void DeIndent() { body.DeIndent(); }
    ...
    void TrimLineEnd() { body.TrimLineEnd(); }
    ...
    void PrintComment(const opString& s) { body.PrintComment(s); }
...
class opFileSectionStream {
   public:
    opFileSectionStream(FileWriteStream& stream)
        : heading(stream), body(stream), footer(stream), o(stream) {}
...
class opFileStream {
   public:
    opFileStream(FileWriteStream& fheader, FileWriteStream& fsource)
        : header(fheader),
          source(fsource),
          headerstream(fheader),
          sourcestream(fsource) {
    ...
    void PrintComment(const opString& s) { header.PrintComment(s); }
...
class opDialectStream : public opFileStream {
   public:
    ...
    void Indent() { header.heading.Indent(); }

    void DeIndent() { header.heading.DeIndent(); }
    ...
    void PrintComment(const opString& s) { header.heading.PrintComment(s); }
};
...
class opXmlStream {
   public:
    ...
    void CompactStartTag(const opString& tag) { stream << '<' << tag << '>'; }
    ...
    void Write(const opString& s) { stream << s; }
...
template <>
inline opXmlStream& operator<<<endlinestruct>(opXmlStream& stream,
                                              const endlinestruct& input) {
    stream.EndLine();
    return stream;
}
...
template <>
inline opXmlStream& operator<<<opString>(opXmlStream& stream,
                                         const opString& input) {
    opString in = input;
...
template <>
inline opXmlStream& operator<<<string>(opXmlStream& stream,
                                       const string& input) {
    opString in = input;
...
class opXmlTextTag {
   public:
    ...
    ~opXmlTextTag() { stream.CompactEndTag(tag); }
...
class DirectiveOverride {
   public:
    ...
    ~DirectiveOverride() { opStringStream::SetLineDirectives(oldvalue); }
...
class LineOverride {
   public:
    explicit LineOverride(opSectionStream& s, opNode* sourcenode)
        : stream(s.body) {
    ...
    ~LineOverride() { stream.ResetOverrideLine(); }
...
class LineOrigin {
   public:
    explicit LineOrigin(opSectionStream& s, opNode* sourcenode)
        : stream(s.body) {
    ...
    ~LineOrigin() { stream.ResetLineOrigin(); }
...
class CommentMode {
   public:
    ...
    ~CommentMode() { stream.ignoreredirects = oldvalue; }
...
class ForceNewlines {
   public:
    ...
    ~ForceNewlines() { stream.forcenewlines = false; }
...
// template<>
// inline opStringStream& operator << <TerminalNode*&>(opStringStream &oin,
// TerminalNode*& node)
// {
// 	Token id = node->GetId();
//
...
// 	{
// 		Log("hit comment, compact = " +
// opString(opParameters::Get().Compact) + ", forcecomments = " +
// opString(opStringStream::ForceComments) );
// 	}
//
