...
inline bool Namespaces<Parent>::Parse() {
    PARSE_START;
    { FindNamespaces(); }
    PARSE_END;
}
...
inline void Namespaces<Parent>::FindNamespaces() {
    ...
    {
        HIT(T_NAMESPACE) {
            stackedcontext<NamespaceNode> newNode =
                opNode::Make<NamespaceNode>(T_NAMESPACE);
            ...
            if (this->IsCurrent(T_ASSIGN)) {
                ...
                stackedcontext<NamespaceAliasNode> alias =
                    opNode::Transform<NamespaceAliasNode>(newNode);
                ...
                while (this->IsCurrent(T_ID)) {
                    stacked<TerminalNode> scope =
                        opNode::Expect<TerminalNode>(T_ID);
                ...
                stacked<BraceBlockNode> bbn =
                    opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
                stacked<NamespaceBlockNode> nbn =
                    opNode::Transform<NamespaceBlockNode>(bbn);
                newNode->SetBody(*nbn);
...
template <class Parent>
inline bool UsingNamespaceKeywords<Parent>::Parse() {
    PARSE_START;
    ...
    LOOP_START(G_USING_NAMESPACE_KEYWORD) {
        HIT(T_USING) {
            if (this->PeekUncleaned(T_NAMESPACE)) {
                stackedcontext<UsingNamespaceKeywordNode> newNode =
                    opNode::Make<UsingNamespaceKeywordNode>(T_USING);
...
template <class Parent>
inline bool Usings<Parent>::Parse() {
    PARSE_START;
    { FindUsings(); }
    PARSE_END;
}
...
inline void Usings<Parent>::FindUsings() {
    LOOP_START(G_USING) {
        HIT(T_USING) {
            stackedcontext<UsingNode> newNode =
                opNode::Make<UsingNode>(T_USING);
...
inline void Blocks<Parent>::FindMatching() {
    ...
    setexceptionmode mode(opException::ParseException);  // TEST: I don't know
    ...
    LOOP_START(Grammar);
    {
        HIT(Left) {
            ...
            // we must delete the current node (Left), our position has moved
            // forward by one
...
inline void Blocks<Parent>::FindBrackets() {
    FindMatching<G_BRACKET_BLOCK, T_LEFT_BRACKET, T_RIGHT_BRACKET,
                 BracketBlockNode>();
}

template <class Parent>
inline void Blocks<Parent>::FindAngles() {
    FindMatching<G_ANGLED_BLOCK, T_LESS_THAN, T_GREATER_THAN,
                 AngledBlockNode>();
}
...
template <class Parent>
inline bool Pointers<Parent>::Parse() {
    PARSE_START;
    { FindPointers(); }
    PARSE_END;
}
...
inline void Pointers<Parent>::FindPointers() {
    ...
    {
        HIT(T_STAR) {
            stackedcontext<PointerNode> newNode =
                opNode::Make<PointerNode>(T_STAR);
            ...
            while (this->IsCurrent(T_STAR)) {
                ...
                // const volatile
                if (type == PointerNode::Volatile && this->IsCurrent(T_CONST)) {
                    this->Erase(T_CONST);
            ...
            stacked<opNode> type = this->ReverseExpectOr(
                G_TEMPLATE_TYPE, G_SCOPE, T_ID, T_BASIC_TYPE,
                G_FUNDAMENTAL_TYPE, *firststar);
...
template <class Parent>
inline bool MemberPointers<Parent>::Parse() {
    PARSE_START;
    { FindMemberPointers(); }
    PARSE_END;
}
...
template <class Parent>
inline bool Operators<Parent>::Parse() {
    PARSE_START;
    { FindOperators(); }
    PARSE_END;
}
...
inline void Operators<Parent>::FindOperators() {
    ...
    {
        HIT(T_OPERATOR) {
            stackedcontext<OperatorNode> newNode =
                opNode::Make<OperatorNode>(T_OPERATOR);
            ...
            if (this->IsCurrent(G_PAREN_BLOCK)) {
                ...
                stacked<ParenBlockNode> paren =
                    opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
...
template <class Parent>
inline bool Modifiers<Parent>::Parse() {
    PARSE_START;
    { FindModifiers(); }
    PARSE_END;
}
...
template <class Parent>
inline bool ValuedModifiers<Parent>::Parse() {
    PARSE_START;
    { FindValuedModifiers(); }
    PARSE_END;
}
    ...
    LOOP_START(G_VALUED_MODIFIER);
    {
        HIT(T_ID) {
            ...
            if (this->Peek(G_PAREN_BLOCK) &&
                ...
                stackedcontext<ValuedModifierNode> newNode =
                    opNode::Make<ValuedModifierNode>(T_ID);

                stacked<TerminalNode> modifier =
                    opNode::Expect<TerminalNode>(T_ID);
                ...
                stacked<ParenBlockNode> paren =
                    opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
                stacked<ValuedModifierArgumentNode> arguments =
                    opNode::Transform<ValuedModifierArgumentNode>(paren);
...
template <class Parent>
inline bool Functions<Parent>::Parse() {
    PARSE_START;
    { FindFunctions(); }
    PARSE_END;
}
...
inline void Functions<Parent>::FindFunctions() {
    ...
    {
        HIT(G_PAREN_BLOCK) {
            stackedcontext<FunctionNode> newNode =
                opNode::Make<FunctionNode>(G_PAREN_BLOCK);

            stacked<ParenBlockNode> Paren =
                opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
            stacked<FunctionArgumentListNode> Arguments =
                opNode::Transform<FunctionArgumentListNode>(Paren);

            newNode->SetArguments(*Arguments);

            // now look backwards
            stacked<opNode> Name = this->ReverseExpectOr(
                G_OPERATOR, G_TEMPLATE_TYPE, T_ID, *Arguments);
            ...
            // NOTE: we just look for assignment = #, = ID, we dont look for
            // null or anything like that
            if (this->IsCurrent(T_ASSIGN)) {
                this->Erase(T_ASSIGN);

                stacked<TerminalNode> assignment =
                    opNode::ExpectOr<TerminalNode>(T_NUMBER, T_ID);
...
template <class Parent>
inline bool FunctionDefinitions<Parent>::Parse() {
    PARSE_START;
...
inline void FunctionDefinitions<Parent>::FindFunctionDefinitions() {
    ...
    {
        HIT(G_FUNCTION) {
            stacked<FunctionNode> Function =
                opNode::Expect<FunctionNode>(G_FUNCTION);
            ...
            if (bFunctionDefinition) {
                ...
                stackedcontext<FunctionDefinitionNode> newNode =
            ...
            // must be a function prototype
                ...
                // create a new node from the block
                stackedcontext<FunctionPrototypeNode> newNode =
                    NEWNODE(FunctionPrototypeNode());
                newNode->CopyBasics(*Function);
...
template <class Parent>
inline stacked<opNode> FunctionDefinitions<Parent>::GetReturnType(
    opNode* after) {
    // find the return type
    return this->ReverseExpectOr(G_TEMPLATE_TYPE, G_SCOPE, G_POINTER,
                                 G_POINTER_MEMBER, G_REFERENCE, T_ID,
                                 T_BASIC_TYPE, G_FUNDAMENTAL_TYPE, after);
}
...
template <class Parent>
inline bool ConstructorDefinitions<Parent>::Parse() {
    PARSE_START;
...
inline void ConstructorDefinitions<Parent>::FindConstructorDefinitions() {
    LOOP_START(G_CONSTRUCTOR_DEFINITION);
    {
        HIT(G_CONSTRUCTOR) {
            ...
            stacked<ConstructorNode> Constructor =
                opNode::Expect<ConstructorNode>(G_CONSTRUCTOR);
            ...
            bool bdefinition =
                this->IsCurrent(T_COLON) || this->IsCurrent(G_BRACE_BLOCK);

            if (bdefinition) {
                // create a new node from the block
                stackedcontext<ConstructorDefinitionNode> newNode =
                    NEWNODE(ConstructorDefinitionNode());
                newNode->CopyBasics(*Constructor);

                // optional initializer list
                if (this->IsCurrent(T_COLON)) {
                    stacked<ConstructorInitializerListNode> initializers =
                        opNode::PushUntil<ConstructorInitializerListNode>(
                            G_BRACE_BLOCK);
            ...
            // must be a function prototype
                ...
                // create a new node from the block
                stackedcontext<ConstructorPrototypeNode> newNode =
                    NEWNODE(ConstructorPrototypeNode());
                newNode->CopyBasics(*Constructor);
...
template <class Parent>
inline bool DestructorDefinitions<Parent>::Parse() {
    PARSE_START;
...
inline void DestructorDefinitions<Parent>::FindDestructorDefinitions() {
    LOOP_START(G_DESTRUCTOR_DEFINITION);
    {
        HIT(G_DESTRUCTOR) {
            // find the constructor
            stacked<DestructorNode> Destructor =
                opNode::Expect<DestructorNode>(G_DESTRUCTOR);
            ...
            if (bdefinition) {
                // create a new node from the block
                stackedcontext<DestructorDefinitionNode> newNode =
                    NEWNODE(DestructorDefinitionNode());
            ...
            // must be a function prototype
                ...
                // create a new node from the block
                stackedcontext<DestructorPrototypeNode> newNode =
                    NEWNODE(DestructorPrototypeNode());
                newNode->CopyBasics(*Destructor);
...
template <class Parent>
inline bool OPEnums<Parent>::Parse() {
    PARSE_START;
    { FindOPEnums(); }
    PARSE_END;
}
...
inline void OPEnums<Parent>::FindOPEnums() {
    ...
    {
        HIT(T_OPENUM) {
            stackedcontext<OPEnumNode> newNode =
                opNode::Make<OPEnumNode>(T_OPENUM);

            stacked<TerminalNode> identifier =
                opNode::Expect<TerminalNode>(T_OPENUM);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<OPEnumBodyNode> eobn =
                opNode::Transform<OPEnumBodyNode>(bbn);
...
template <class Parent>
inline bool OPObjects<Parent>::Parse() {
    PARSE_START;
    { FindOPObjects(); }
    PARSE_END;
}
...
inline void OPObjects<Parent>::FindOPObjects() {
    ...
    {
        HIT(T_OPOBJECT) {
            stackedcontext<OPObjectNode> newNode =
                opNode::Make<OPObjectNode>(T_OPOBJECT);

            stacked<opNode> category = opNode::Expect(T_OPOBJECT);

            newNode->SetCategory(*category);

            // I don't want to save this, because untransformed one is
            // disallowed
            category.Delete();
            ...
            if (this->IsCurrent(G_CPLUSPLUS)) {
                stacked<CPlusPlusNode> middleModifiers =
                    opNode::Expect<CPlusPlusNode>(G_CPLUSPLUS);
            ...
            // add inheritance
            stacked<OPObjectInheritanceNode> i =
                opNode::PushUntil<OPObjectInheritanceNode>(G_BRACE_BLOCK);
            i->SetObject(*newNode);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<OPObjectBodyNode> ocbn =
                opNode::Transform<OPObjectBodyNode>(bbn);
...
template <class Parent>
inline bool States<Parent>::Parse() {
    PARSE_START;
...
inline void States<Parent>::FindStates() {
    ...
    HIT(T_STATE) {
        ...
        this->EatWhitespaceAndComments();
        if (this->IsCurrent(T_SEMICOLON)) this->Erase(T_SEMICOLON);
...
template <class Parent>
inline bool TemplateTypes<Parent>::Parse() {
    PARSE_START;
    { FindTemplateTypes(); }
    PARSE_END;
}
...
inline void TemplateTypes<Parent>::FindTemplateTypes() {
    ...
    {
        HIT(G_ANGLED_BLOCK) {
            stackedcontext<TemplateTypeNode> newNode =
                opNode::Make<TemplateTypeNode>(G_ANGLED_BLOCK);

            // Grab the braces.
            stacked<AngledBlockNode> braces =
                opNode::Expect<AngledBlockNode>(G_ANGLED_BLOCK);
            stacked<TemplateTypeBodyNode> body =
                opNode::Transform<TemplateTypeBodyNode>(braces);

            newNode->SetBody(*body);

            // Grab the id.
            stacked<TerminalNode> id =
                opNode::ReverseExpect<TerminalNode>(T_ID, *body);
...
template <class Parent>
inline bool References<Parent>::Parse() {
    PARSE_START;
    { FindReferences(); }
    PARSE_END;
}
...
inline void References<Parent>::FindReferences() {
    LOOP_START(G_REFERENCE) {
        HIT(T_AMPERSAND) {
            stackedcontext<ReferenceNode> newNode =
                opNode::Make<ReferenceNode>(T_AMPERSAND);

            stacked<TerminalNode> ampersand =
                opNode::Expect<TerminalNode>(T_AMPERSAND);

            stacked<opNode> Type = this->ReverseExpectOr(
                G_POINTER, G_SCOPE, G_TEMPLATE_TYPE, T_ID, T_BASIC_TYPE,
                G_FUNDAMENTAL_TYPE, *ampersand);
...
template <class Parent>
inline bool Arrays<Parent>::Parse() {
    PARSE_START;
    { FindArrays(); }
    PARSE_END;
}
...
inline void Arrays<Parent>::FindArrays() {
    LOOP_START(G_ARRAY) {
        HIT(G_BRACKET_BLOCK) {
            ...
            stackedcontext<opNode> newNode =
                opNode::Make<opNode>(G_BRACKET_BLOCK);

            opArray<stacked<BracketBlockNode> > brackets;

            while (this->IsCurrent(G_BRACKET_BLOCK)) {
                stacked<BracketBlockNode> b =
                    opNode::Expect<BracketBlockNode>(G_BRACKET_BLOCK);
                brackets.push_back(b);
            }

            if (this->IsPrevious(G_FUNDAMENTAL_TYPE) ||
                this->IsPrevious(T_BASIC_TYPE) || this->IsPrevious(G_POINTER) ||
                this->IsPrevious(G_REFERENCE) ||
                this->IsPrevious(G_TEMPLATE_TYPE) ||
                this->IsPrevious(G_SCOPE)) {
                stackedcontext<TypeArrayNode> node =
                    opNode::Transform<TypeArrayNode>(newNode);

                // get the type
                stacked<opNode> Type = opNode::ReverseExpectOr<opNode>(
                    G_FUNDAMENTAL_TYPE, T_BASIC_TYPE, G_POINTER, G_REFERENCE,
                    G_TEMPLATE_TYPE, G_SCOPE, *brackets[0]);
                ...
                // G_ARRAY
                stackedcontext<ArrayNode> node =
                    opNode::Transform<ArrayNode>(newNode);

                // get the name
                stacked<TerminalNode> Name =
                    opNode::ReverseExpect<TerminalNode>(T_ID, *brackets[0]);
...
template <class Parent>
template <typename T>
inline void Arrays<Parent>::AppendBrackets(
    opArray<stacked<BracketBlockNode> >& brackets, T* node) {
...
template <class Parent>
inline void VisibilityLabels<Parent>::FindVisibilityLabels() {
    ...
    LOOP_START(G_VISIBILITY_LABEL) {
        HIT(token) {
            if (this->IsCurrent(token) && this->Peek(T_COLON)) {
                stackedcontext<VisibilityLabelNode> newNode =
                    opNode::Make<VisibilityLabelNode>(token);

                stacked<TerminalNode> Label =
                    opNode::Expect<TerminalNode>(token);
                this->Erase(T_COLON);
...
template <class Parent>
inline bool Scopes<Parent>::Parse() {
    PARSE_START;
    { FindScopes(); }
    PARSE_END;
}
...
inline void Scopes<Parent>::FindScopes() {
    LOOP_START(G_SCOPE) {
        HIT(T_SCOPE_RESOLUTION) {
            ...
            stackedcontext<ScopeNode> scope =
                opNode::Make<ScopeNode>(T_SCOPE_RESOLUTION);

            stacked<TerminalNode> scoperesolution =
                opNode::Expect<TerminalNode>(T_SCOPE_RESOLUTION);
            ...
            if (this->IsPrevious(T_ID) || this->IsPrevious(G_TEMPLATE_TYPE)) {
                ...
                stacked<opNode> name = this->ReverseExpectOr(
                    T_ID, G_TEMPLATE_TYPE, *scoperesolution);
...
template <class Parent>
inline bool Scopes<Parent>::CheckScopePointer(stacked<ScopeNode>& scope,
                                              opArray<stacked<opNode> >& scopes,
                                              bool bGlobal) {
    if (this->IsCurrent(T_STAR)) {
        stacked<ScopePointerNode> scopepointer =
            opNode::Transform<ScopePointerNode>(scope);
...
template <class Parent>
inline bool PointerMembers<Parent>::Parse() {
    PARSE_START;
    { FindPointerMembers(); }
    PARSE_END;
}
...
inline void PointerMembers<Parent>::FindPointerMembers() {
    ...
    {
        HIT(G_SCOPE_POINTER) {
            stackedcontext<PointerMemberNode> newNode =
                opNode::Make<PointerMemberNode>(G_SCOPE_POINTER);

            stacked<ScopePointerNode> scopepointer =
                opNode::Expect<ScopePointerNode>(G_SCOPE_POINTER);

            newNode->SetScope(*scopepointer);

            stacked<opNode> type = opNode::ReverseExpectOr(
                T_ID, G_SCOPE, G_TEMPLATE_TYPE, G_POINTER, G_REFERENCE,
                T_BASIC_TYPE, G_FUNDAMENTAL_TYPE, *scopepointer);
...
template <class Parent>
inline bool Typenames<Parent>::Parse() {
    PARSE_START;
    { FindTypenames(); }
    PARSE_END;
}
...
inline void Typenames<Parent>::FindTypenames() {
    LOOP_START(G_TYPENAME) {
        HIT(T_TYPENAME) {
            stackedcontext<TypenameNode> newNode =
                opNode::Make<TypenameNode>(T_TYPENAME);

            this->Erase(T_TYPENAME);

            opNode* name = this->ExpectOr(T_ID, G_TEMPLATE_TYPE, G_SCOPE,
                                          G_POINTER, G_REFERENCE);
...
template <class Parent>
inline bool FunctionPointers<Parent>::Parse() {
    PARSE_START;
...
inline void FunctionPointers<Parent>::FindFunctionPointers() {
    LOOP_START(G_FUNCTION_POINTER) {
        HIT(G_PAREN_BLOCK) {
            ...
            stackedcontext<FunctionPointerNode> newNode =
                opNode::Make<FunctionPointerNode>(G_PAREN_BLOCK);

            stacked<ParenBlockNode> DeclParen =
                opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
            stacked<FunctionPointerDeclNode> Decl =
                opNode::Transform<FunctionPointerDeclNode>(DeclParen);
            newNode->SetDecl(*Decl);
            newNode->AppendNode(Decl);

            stacked<ParenBlockNode> Args =
                opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
            ...
            stacked<opNode> ReturnType =
                this->ExpectOr(G_SCOPE, G_TEMPLATE_TYPE, G_POINTER, G_REFERENCE,
                               T_ID, T_BASIC_TYPE, G_FUNDAMENTAL_TYPE);
...
template <class Parent>
inline bool OPDefines<Parent>::Parse() {
    PARSE_START;
    { FindOPDefines(); }
    PARSE_END;
}
...
inline void OPDefines<Parent>::FindOPDefines() {
    LOOP_START(G_OPDEFINE) {
        HIT(T_OPDEFINE) {
            stackedcontext<OPDefineNode> newNode =
                opNode::Make<OPDefineNode>(T_OPDEFINE);
            ...
            if (this->IsCurrent(G_PAREN_BLOCK)) {
                stacked<ParenBlockNode> arguments =
                    opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
                newNode->SetArguments(*arguments);
                newNode->AppendNode(arguments);
            }
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<OPDefineBodyNode> body =
                opNode::Transform<OPDefineBodyNode>(bbn);
            newNode->SetBody(*body);
            newNode->AppendNode(body);
...
template <class Parent>
inline bool Preprocessors<Parent>::Parse() {
    PARSE_START;
    { FindAllPounds(); }
    PARSE_END;
}
...
template <class Parent>
inline void Preprocessors<Parent>::FindAllPounds() {
    FindPoundDefines();
    FindPoundIf();
    FindPoundIfDef();
    FindPoundIfnDef();
...
inline void Preprocessors<Parent>::FindDirective(const string& directivename) {
    LOOP_START(Grammar) {
        HIT(HitToken) {
            ...
            if (!this->IsPrevious(T_NEWLINE) &&
                this->GetPosition() != this->GetBegin())
                opError::MessageError(*newNode,
                                      "Preprocessor definitions must not be "
                                      "preceeded by any tokens on a line "
                                      "except whitespace.");
            ...
            while (!bDone) {
                this->PushUntilOrAdd(*newNode, T_NEWLINE, T_COMMENT, T_CCOMMENT,
                                     T_EOF, T_CONTINUELINE);
                ...
                if (this->CurrentNode()) {
                    if (this->CurrentNode()->GetId() == T_NEWLINE ||
                        ...
                        stacked<TerminalNode> continueline =
                            opNode::Expect<TerminalNode>(T_CONTINUELINE);
...
template <class Parent>
inline void Preprocessors<Parent>::FindPoundIf() {
    FindDirective<G_POUND_IF, T_POUND_IF, PoundIfNode>("if");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundIfDef() {
    FindDirective<G_POUND_IFDEF, T_POUND_IFDEF, PoundIfdefNode>("ifdef");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundIfnDef() {
    FindDirective<G_POUND_IFNDEF, T_POUND_IFNDEF, PoundIfndefNode>("ifndef");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundEndIf() {
    FindDirective<G_POUND_ENDIF, T_POUND_ENDIF, PoundEndifNode>("endif");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundInclude() {
    FindDirective<G_POUND_INCLUDE, T_POUND_INCLUDE, PoundIncludeNode>(
        "include");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundElse() {
    FindDirective<G_POUND_ELSE, T_POUND_ELSE, PoundElseNode>("else");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundError() {
    FindDirective<G_POUND_ERROR, T_POUND_ERROR, PoundErrorNode>("error");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundImport() {
    FindDirective<G_POUND_IMPORT, T_POUND_IMPORT, PoundImportNode>("import");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundLine() {
    FindDirective<G_POUND_LINE, T_POUND_LINE, PoundLineNode>("line");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundPragma() {
    FindDirective<G_POUND_PRAGMA, T_POUND_PRAGMA, PoundPragmaNode>("pragma");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundUnDef() {
    FindDirective<G_POUND_UNDEF, T_POUND_UNDEF, PoundUndefNode>("undef");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundUsing() {
    FindDirective<G_POUND_USING, T_POUND_USING, PoundUsingNode>("using");
}

template <class Parent>
inline void Preprocessors<Parent>::FindPoundWarning() {
    FindDirective<G_POUND_WARNING, T_POUND_WARNING, PoundWarningNode>(
        "warning");
}
...
template <class Parent>
inline bool OPIncludes<Parent>::PreParse() {
    PREPARSE_START;
    { FindOPIncludes(); }
    PREPARSE_END;
}
...
inline void OPIncludes<Parent>::FindOPIncludes() {
    LOOP_START(G_OPINCLUDE) {
        HIT(T_OPINCLUDE) {
            stackedcontext<OPIncludeNode> newNode =
                opNode::Make<OPIncludeNode>(T_OPINCLUDE);

            this->Erase(T_OPINCLUDE);

            this->EatWhitespaceAndComments();

            stacked<TerminalNode> FileName =
                opNode::Expect<TerminalNode>(T_STRING);
...
template <class Parent>
inline void ExpandCalls<Parent>::FindExpandCalls() {
    LOOP_START(G_EXPAND_CALL);
    {
        HIT(T_EXPAND) {
            stackedcontext<ExpandCallNode> newNode =
                opNode::Make<ExpandCallNode>(T_EXPAND);
            ...
            if (this->IsCurrent(G_PAREN_BLOCK)) {
                stacked<ParenBlockNode> args =
                    opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
                stacked<ExpandCallArgumentListNode> arguments =
                    opNode::Transform<ExpandCallArgumentListNode>(args);
...
template <class Parent>
inline void OPMacros<Parent>::FindOPMacros() {
    LOOP_START(G_OPMACRO) {
        HIT(T_OPMACRO) {
            stackedcontext<OPMacroNode> newNode =
                opNode::Make<OPMacroNode>(T_OPMACRO);
            ...
            if (this->IsCurrent(G_PAREN_BLOCK)) {
                stacked<ParenBlockNode> args =
                    opNode::Expect<ParenBlockNode>(G_PAREN_BLOCK);
                stacked<OPMacroArgumentListNode> arguments =
                    opNode::Transform<OPMacroArgumentListNode>(args);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<OPMacroBodyNode> body =
                opNode::Transform<OPMacroBodyNode>(bbn);
            newNode->SetBody(*body);
            newNode->AppendNode(body);
...
template <class Parent>
inline bool CPlusPluses<Parent>::Parse() {
    PARSE_START;
    { FindCPlusPluses(); }
    PARSE_END;
}
...
inline void CPlusPluses<Parent>::FindCPlusPluses() {
    LOOP_START(G_CPLUSPLUS) {
        HIT(T_CPLUSPLUS) {
            stackedcontext<CPlusPlusNode> newNode =
                opNode::Make<CPlusPlusNode>(T_CPLUSPLUS);
            ...
            stacked<BraceBlockNode> bbn =
                opNode::Expect<BraceBlockNode>(G_BRACE_BLOCK);
            stacked<CPlusPlusBodyNode> body =
                opNode::Transform<CPlusPlusBodyNode>(bbn);
...
template <class Parent>
inline void FundamentalTypes<Parent>::FindSigned() {
    LOOP_START(G_FUNDAMENTAL_TYPE) {
        HIT(T_SIGNED) {
            stackedcontext<FundamentalTypeNode> newNode =
                opNode::Make<FundamentalTypeNode>(T_SIGNED);

            this->Erase(T_SIGNED);

            stacked<TerminalNode> type =
                opNode::Expect<TerminalNode>(T_BASIC_TYPE);
...
template <class Parent>
inline void FundamentalTypes<Parent>::FindUnsigned() {
    LOOP_START(G_FUNDAMENTAL_TYPE) {
        HIT(T_UNSIGNED) {
            stackedcontext<FundamentalTypeNode> newNode =
                opNode::Make<FundamentalTypeNode>(T_UNSIGNED);
    ...
    LOOP_START(G_CONSTRUCTOR) {
        HIT(G_FUNCTION) {
            FunctionNode* function =
                node_cast<FunctionNode>(this->CurrentNode());
            TerminalNode* fname = node_cast<TerminalNode>(function->GetName());

            if (fname && fname->GetValue() == classname) {
                stackedcontext<ConstructorNode> ctor =
                    opNode::Make<ConstructorNode>(G_FUNCTION);
                stacked<FunctionNode> f =
                    opNode::Expect<FunctionNode>(G_FUNCTION);
    ...
    LOOP_START(G_DESTRUCTOR) {
        HIT(G_FUNCTION) {
            ...
            if (fname && fname->GetValue() == classname &&
                ...
                stackedcontext<DestructorNode> dtor =
                    opNode::Make<DestructorNode>(G_FUNCTION);
                stacked<FunctionNode> f =
                    opNode::Expect<FunctionNode>(G_FUNCTION);
                dtor->SetFunction(*f);
                ...
                if (!f->GetArguments()->IsEmpty())
                    opError::MessageError(
                        *f, "Destructors must have a void parameter list.");
...
template <class Parent>
inline bool Friends<Parent>::Parse() {
    PARSE_START;
    { FindFriends(); }
    PARSE_END;
}
...
inline void Friends<Parent>::FindFriends() {
    ...
    {
        HIT(T_FRIEND) {
            stackedcontext<FriendNode> newNode =
                opNode::Make<FriendNode>(T_FRIEND);

            this->Erase(T_FRIEND);

            stacked<opNode> first = opNode::ExpectOr(
                T_ID, G_SCOPE, G_TEMPLATE_TYPE, T_CLASS, T_STRUCT,
                G_FUNCTION_PROTOTYPE, G_FUNCTION_DEFINITION);
            ...
            if (tok == T_CLASS || tok == T_STRUCT) {
                ...
                stacked<opNode> name =
                    opNode::ExpectOr(T_ID, G_SCOPE, G_TEMPLATE_TYPE);
...
template <class Parent>
inline bool Typedefs<Parent>::Parse() {
    PARSE_START;
    { FindTypedefs(); }
    PARSE_END;
}
...
inline void Typedefs<Parent>::FindTypedefs() {
    ...
    {
        HIT(T_TYPEDEF) {
            stackedcontext<TypedefNode> newNode =
                opNode::Make<TypedefNode>(T_TYPEDEF);

            this->Erase(T_TYPEDEF);

            if (this->IsCurrent(T_VOLATILE) || this->IsCurrent(T_CONST)) {
                stacked<TerminalNode> modifier =
                    opNode::ExpectOr<TerminalNode>(T_VOLATILE, T_CONST);

                newNode->SetModifier(*modifier);
                newNode->AppendNode(modifier);
            }

            stacked<opNode> type = opNode::ExpectOr(
                T_ID, G_POINTER, G_REFERENCE, G_TEMPLATE_TYPE, G_SCOPE,
                T_BASIC_TYPE, G_FUNDAMENTAL_TYPE, G_TYPE_ARRAY, G_ARRAY);
...
template <class Parent>
inline bool TemplateDecls<Parent>::Parse() {
    PARSE_START;
    { FindTemplateDecls(); }
    PARSE_END;
}
...
inline void TemplateDecls<Parent>::FindTemplateDecls() {
    ...
    {
        HIT(T_TEMPLATE) {
            stackedcontext<TemplateDeclNode> newNode =
                opNode::Make<TemplateDeclNode>(T_TEMPLATE);
...
template <class Parent>
inline bool CPPConstructs<Parent>::Parse() {
    PARSE_START;
    { FindCPPConstructs(); }
    PARSE_END;
}
    ...
    LOOP_START(Grammar);
    {
        HIT(Hit) {
            ...
            if (this->IsCurrent(T_ID)) {
                ...
                stacked<CPPConstructInheritanceNode> i =
                    opNode::PushUntil<CPPConstructInheritanceNode>(
                        G_BRACE_BLOCK);
...
template <class Parent>
inline bool Templated<Parent>::Parse() {
    PARSE_START;
    { FindTemplated(); }
    PARSE_END;
}
...
inline void Templated<Parent>::FindTemplated() {
    ...
    {
        HIT(G_TEMPLATE_DECL) {
            stackedcontext<TemplatedNode> newNode =
                opNode::Make<TemplatedNode>(G_TEMPLATE_DECL);
            stacked<TemplateDeclNode> theTemplate =
                opNode::Expect<TemplateDeclNode>(G_TEMPLATE_DECL);

            newNode->SetTheTemplate(*theTemplate);
            newNode->AppendNode(theTemplate);

            stacked<opNode> templated = opNode::ExpectOr(
                T_SEMICOLON, G_CLASS, G_STRUCT, G_UNION, G_FUNCTION_DEFINITION,
                G_FUNCTION_PROTOTYPE, G_CONSTRUCTOR_DEFINITION,
