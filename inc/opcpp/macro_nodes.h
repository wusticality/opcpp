...
class ExpandCallArgumentNode : public ExpandCallArgumentNodeParent {
   public:
    DECLARE_NODE(ExpandCallArgumentNode, ExpandCallArgumentNodeParent,
...
class ExpandCallArgumentListNode : public ExpandCallArgumentListNodeParent {
   public:
    DECLARE_NODE(ExpandCallArgumentListNode, ExpandCallArgumentListNodeParent,
                 G_EXPAND_CALL_ARGUMENT_LIST);

    bool PreParse();

    const vector<ExpandCallArgumentNode*>& GetArguments() { return Arguments; }
...
class ExpandCallNode : public opNode {
   public:
    ...
    ExpandCallArgumentListNode* GetArguments() { return Arguments; }
    ...
    // protected:
    bool Expand(opSymbolTracker& tracker, opNode::iterator expandit,
                opNode* parent);
...
class ExpandableNode : public opNode {
   ...
   private:
    /**** recursive functions ****/
    void DoReplacement(opNode* currentnode, const opString& matchname,
...
// this class is used to wrap expandable arguments, it is collapsed after
// substitution
class ExpandableArgumentNode : public opNode {
   public:
    ...
    void SetValue(const opString& invalue) { value = invalue; }

    opString GetTreeValue() { return value; }
...
class ConcatenationArgumentOperatorNode : public opNode {
   public:
    DECLARE_NODE(ConcatenationArgumentOperatorNode, opNode,
                 G_CONCATENATION_ARGUMENT_OPERATOR);

    void Init() { Argument = NULL; }

    void SetArgument(opNode* innode) { Argument = innode; }

    opNode* GetArgument() { return Argument; }
...
class ConcatenationOperatorNode : public opNode {
   public:
    ...
    void SetLeft(ConcatenationArgumentOperatorNode* innode) { Left = innode; }

    void SetRight(ConcatenationArgumentOperatorNode* innode) { Right = innode; }

    ConcatenationArgumentOperatorNode* GetLeft() { return Left; }

    ConcatenationArgumentOperatorNode* GetRight() { return Right; }
...
class SingleQuoteOperatorNode : public opNode {
   public:
    ...
    void CloneNode(SingleQuoteOperatorNode* node) { CloneChildren(node); }
...
class DoubleQuoteOperatorNode : public opNode {
   public:
    ...
    void CloneNode(DoubleQuoteOperatorNode* node) { CloneChildren(node); }
