...
inline void opNodeBase::CopyLineNum(opNode* n) { line = n->line; }

inline void opNodeBase::CopyFile(opNode* n) { file = n->file; }
...
inline void opNodeBase::RemoveNode(iterator index) {
    ...
    if (GetPosition() == index) IncrementPosition();
...
inline void opNodeBase::RemoveCurrentNode() { RemoveNode(pos); }

// Removes first node
inline void opNodeBase::RemoveFirstNode() { RemoveNode(GetBegin()); }

// Removes last node
inline void opNodeBase::RemoveLastNode() { RemoveNode(GetLast()); }
...
inline void opNodeBase::DeleteNode(iterator index) {
    ...
    if (index == GetPosition()) IncrementPosition();
...
inline void opNodeBase::DeleteNode(opNode* node) {
    ...
    if (i != GetEnd()) DeleteNode(i);
}

// deletes node from children deque at "pos"
inline void opNodeBase::DeleteCurrentNode() { DeleteNode(pos); }

// deletes first node
inline void opNodeBase::DeleteFirstNode() { DeleteNode(GetBegin()); }

// deletes last node
inline void opNodeBase::DeleteLastNode() { DeleteNode(GetLast()); }
...
inline stacked<opNode> opNodeBase::PopCurrentNode() { return PopNode(pos); }

inline stacked<opNode> opNodeBase::PopFirstNode() {
    return PopNode(GetBegin());
}

inline stacked<opNode> opNodeBase::PopLastNode() { return PopNode(GetLast()); }

// move nodes
inline void opNodeBase::MoveNodes(iterator index, opNode* node, iterator start,
                                  iterator end) {
    // switch parent
    iterator it = start;
    while (it != end) {
...
inline opNode* opNodeBase::PreviousNode() {
    ...
    if (lastpos == GetBegin()) return NULL;
...
inline bool opNode::IsWhitespace() {
    ...
    if (tok == T_WHITESPACE || tok == T_NEWLINE) return true;
...
inline bool opNode::IsComment() {
    ...
    if (tok == T_CCOMMENT || tok == T_COMMENT) return true;
...
inline bool opNode::IsOverloadableOperator() {
    ...
    if (id == T_PLUS || id == T_PLUS_PLUS || id == T_PLUS_EQUALS ||
        id == T_MINUS || id == T_MINUS_MINUS || id == T_MINUS_EQUALS ||
        id == T_STAR || id == T_STAR_EQUALS || id == T_DIVIDE ||
        id == T_DIVIDE_EQUALS || id == T_MODULUS || id == T_MODULUS_EQUALS ||
        id == T_LESS_THAN || id == T_LESS_THAN_OR_EQUAL_TO ||
        id == T_GREATER_THAN || id == T_GREATER_THAN_OR_EQUAL_TO ||
        id == T_NOT_EQUALS || id == T_EQUALS || id == T_NEGATION ||
        id == T_AND || id == T_OR || id == T_LEFT_SHIFT ||
        id == T_LEFT_SHIFT_EQUALS || id == T_RIGHT_SHIFT ||
        id == T_RIGHT_SHIFT_EQUALS || id == T_BITWISE_COMPLEMENT ||
        id == T_AMPERSAND || id == T_BITWISE_AND_EQUALS ||
        id == T_BITWISE_INCLUSIVE_OR || id == T_BITWISE_INCLUSIVE_OR_EQUALS ||
        id == T_BITWISE_EXCLUSIVE_OR || id == T_BITWISE_EXCLUSIVE_OR_EQUALS ||
        id == T_ASSIGN || id == T_ARROW || id == T_ARROW_STAR ||
        id == T_COMMA || id == T_NEW || id == T_DELETE) {
        return true;
    }

    if (id == G_PAREN_BLOCK && IsEmpty()) return true;

    if (id == G_BRACKET_BLOCK && IsEmpty()) return true;
...
inline bool opNode::IgnoreUntilBase() {
    ...
    while (GetPosition() != end) {
        ...
        if (TokenList::Matches(tok)) return true;
...
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<t1> >();
}

template <Token t1, Token t2>
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<t1, TokenListNode<t2> > >();
}

template <Token t1, Token t2, Token t3>
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<
        TokenListNode<t1, TokenListNode<t2, TokenListNode<t3> > > >();
}

template <Token t1, Token t2, Token t3, Token t4>
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<
        t1, TokenListNode<t2, TokenListNode<t3, TokenListNode<t4> > > > >();
}
...
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<
        t1, TokenListNode<
                t2, TokenListNode<
                        t3, TokenListNode<t4, TokenListNode<t5> > > > > >();
}
...
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<
        ...
        TokenListNode<
            ...
            TokenListNode<
                t3, TokenListNode<
                        t4, TokenListNode<t5, TokenListNode<t6> > > > > > >();
}
...
template <Token t1, Token t2, Token t3, Token t4, Token t5, Token t6, Token t7,
          Token t8, Token t9, Token t10, Token t11>
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<
        t1,
        TokenListNode<
...
template <Token t1, Token t2, Token t3, Token t4, Token t5, Token t6, Token t7,
          Token t8, Token t9, Token t10, Token t11, Token t12>
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<
        t1,
        TokenListNode<
...
template <Token t1, Token t2, Token t3, Token t4, Token t5, Token t6, Token t7,
          Token t8, Token t9, Token t10, Token t11, Token t12, Token t13>
inline bool opNode::IgnoreUntil() {
    return IgnoreUntilBase<TokenListNode<
        t1,
        TokenListNode<
...
inline bool opNode::IgnoreUntilOr(Token t1, Token t2, Token t3, Token t4, Token
t5)
{
        vector<Token> tokens;
...
inline bool opNode::IgnoreUntilOr(Token t1, Token t2, Token t3, Token t4, Token
t5, Token t6, Token t7, Token t8, Token t9)
{
        vector<Token> tokens;
...
inline bool opNode::IgnoreUntilOr(Token t1, Token t2, Token t3, Token t4, Token
t5, Token t6, Token t7, Token t8, Token t9, Token t10)
{
        vector<Token> tokens;
...
// an exception
inline opNode* opNode::PushUntilBase(opNode* addto, const vector<Token>& tokens,
    ...
    if (bErrorOnMissing) {
        ...
        if (!IsEmpty()) previous = PreviousNode();

        // delete addto;
        // if this errors it's going to throw an exception
        // or else it will return a valid type
        opError::PrematureError(
            tokens, first ? first : previous ? previous : this, tok == T_EOF);
        return NULL;
    }
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2,
                                   bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                                   bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                                   Token t4, Token t5, bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
    tokens.push_back(t3);
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                                   Token t4, Token t5, Token t6, Token t7,
                                   bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                                   Token t4, Token t5, Token t6, Token t7,
                                   Token t8, Token t9, bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
    tokens.push_back(t3);
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                                   Token t4, Token t5, Token t6, Token t7,
                                   Token t8, Token t9, Token t10, Token t11,
                                   Token t12, bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
    tokens.push_back(t3);
...
inline void opNode::PushUntilOrAdd(opNode* addto, Token t1, Token t2, Token t3,
                                   Token t4, Token t5, Token t6, Token t7,
                                   Token t8, Token t9, Token t10, Token t11,
                                   Token t12, Token t13, bool bErrorOnMissing) {
    vector<Token> tokens;
    tokens.push_back(t1);
    tokens.push_back(t2);
    tokens.push_back(t3);
...
template <class T>
inline stacked<T> opNode::PushUntilOr(Token t1, Token t2,
                                      bool bErrorOnMissing) {
    stacked<T> newNode = NEWNODE(T());
    vector<Token> tokens;
...
template <class T>
inline stacked<T> opNode::PushUntilOr(Token t1, Token t2, Token t3,
                                      bool bErrorOnMissing) {
    stacked<T> newNode = NEWNODE(T());
    vector<Token> tokens;
...
template <class T>
inline stacked<T> opNode::PushUntilOr(Token t1, Token t2, Token t3, Token t4,
                                      bool bErrorOnMissing) {
    stacked<T> newNode = NEWNODE(T());
    vector<Token> tokens;
...
inline stacked<T> opNode::PushUntilOr(Token t1, Token t2, Token t3, Token t4,
                                      Token t5, Token t6, Token t7, Token t8,
                                      Token t9, Token t10, Token t11, Token t12,
                                      Token t13, bool bErrorOnMissing) {
    stacked<T> newNode = NEWNODE(T());
    vector<Token> tokens;
...
inline stacked<T> opNode::PushUntilOr(Token t1, Token t2, Token t3, Token t4,
                                      ...
                                      Token t9, Token t10, Token t11, Token t12,
                                      Token t13, Token t14, Token t15,
                                      Token t16, bool bErrorOnMissing) {
    stacked<T> newNode = NEWNODE(T());
    vector<Token> tokens;
...
inline stacked<T> opNode::PushUntilOr(Token t1, Token t2, Token t3, Token t4,
                                      ...
                                      Token t13, Token t14, Token t15,
                                      Token t16, Token t17, Token t18,
                                      Token t19, bool bErrorOnMissing) {
    stacked<T> newNode = NEWNODE(T());
    vector<Token> tokens;
...
inline stacked<T> opNode::ExpectBase(const vector<Token>& Tokens,
                                     bool bReversed, opNode* after) {
    if (!bReversed) {
        iterator end = GetEnd();
        Token tok;
        ...
        if (find(Tokens.begin(), Tokens.end(), tok) == Tokens.end()) {
            ...
            opError::ReverseExpectError(Tokens, *previous,
                                        after ? after : node);
        }
...
template <class T>
inline stacked<T> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                          opNode* after) {
    vector<Token> tokens;
...
template <class T>
inline stacked<T> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                          Token t4, Token t5, opNode* after) {
    vector<Token> tokens;
...
inline stacked<T> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                          Token t4, Token t5, Token t6,
                                          Token t7, Token t8, opNode* after) {
    vector<Token> tokens;
...
inline stacked<T> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                          Token t4, Token t5, Token t6,
                                          Token t7, Token t8, Token t9,
                                          Token t10, Token t11, opNode* after) {
    vector<Token> tokens;
...
inline stacked<T> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                          ...
                                          Token t7, Token t8, Token t9,
                                          Token t10, Token t11, Token t12,
                                          Token t13, Token t14, opNode* after) {
    vector<Token> tokens;
...
inline stacked<opNode> opNode::ReverseExpectOr(Token t1, Token t2,
                                               opNode* after) {
    return opNode::ReverseExpectOr<opNode>(t1, t2, after);
}

// reverse expect or - non-template 3 tokens
inline stacked<opNode> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                               opNode* after) {
...
inline stacked<opNode> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                               ...
                                               Token t7, Token t8,
                                               opNode* after) {
    return opNode::ReverseExpectOr<opNode>(t1, t2, t3, t4, t5, t6, t7, t8,
                                           after);
}
...
inline stacked<opNode> opNode::ReverseExpectOr(Token t1, Token t2, Token t3,
                                               ...
                                               Token t7, Token t8, Token t9,
                                               opNode* after) {
    return opNode::ReverseExpectOr<opNode>(t1, t2, t3, t4, t5, t6, t7, t8, t9,
                                           after);
}
...
inline stacked<T> opNode::Make(Token t) {
    ...
    if (tok == T_EOF) opError::PrematureError(t, CurrentNode(), true);
...
inline void opNode::AllowOnlyBase(const vector<Token>& Tokens) {
    ...
    while (GetPosition() != end) {
        ...
        if (!TokenFunctions::IsWhitespace(t) && !TokenFunctions::IsComment(t) &&
            t != T_EOF) {
...
inline void opNode::DisallowBoth(Token t1, Token t2) {
    ...
    while (i != end) {
        ...
        if (t == t1) {
            ...
            if (!firstfound) firstfound = node1;
        } else if (t == t2) {
            node2 = *i;

            if (!firstfound) firstfound = node2;
        }
...
inline T* opNode::FindParent() {
    ...
    while (parent) {
        if (T* node = fast_node_cast<T>(parent)) return node;
...
inline T* opNode::FindNode() {
    ...
    while (i != end) {
        if (T* node = fast_node_cast<T>(*i)) return node;
...
template <class T>
inline T* FileNode::Load(const opString& file, opScanner::ScanMode scanmode,
                         bool bIncluded) {
    // std::ifstream  ifs;
    opScanner scanner;
    T* rootNode = NULL;
    ...
    if (!bIncluded) FileTable.push_back(rootNode);
    ...
    // preprocessor
    if (!rootNode->Preprocessor()) return rootNode;

    // pre process
    if (!rootNode->PreProcess()) return rootNode;

    if (!bIncluded) {
        // pre operations
        if (!rootNode->PreOperations()) return rootNode;

        // process
        if (!rootNode->Process()) return rootNode;

        // operations
        if (!rootNode->Operations()) return rootNode;

        // post process
        if (!rootNode->PostProcess()) return rootNode;

        // post operations
        if (!rootNode->PostOperations()) return rootNode;
    }
...
inline void opNode::BuildClassScope(opString& outscope) {
    if (GetParent()) GetParent()->BuildClassScope(outscope);
}

inline opString opNode::GetClassScope() {
    opString scope;
    if (GetParent()) GetParent()->BuildClassScope(scope);
    return scope;
}
