...
class opScanner {
   ...
   private:
    ...
    bool IsFloatFlagToken(const opToken& tok) {
        return tok.Id == T_ID && (tok.Value.Size() == 1) &&
               IsFloatChar(tok.Value.Front());
    }

    // returns true if the token is a decimal and it doesn't have a float
    // specifier
   ...
   public:
    ...
    static bool IsPreprocessorKeyword(const opString& s) {
        if (s == "define" || s == "elif" || s == "else" || s == "endif" ||
            s == "error" || s == "if" || s == "ifdef" || s == "ifndef" ||
            s == "import" || s == "include" || s == "line" || s == "pragma" ||
            s == "undef" || s == "using" || s == "warning") {
            return true;
        }
    ...
    static bool IsStandardBasicType(const opString& s) {
        if (s == "char" || s == "bool" || s == "short" || s == "long" ||
            s == "float" || s == "double" || s == "wchar_t" || s == "int" ||
            s == "void" || s == "long long" || s == "long double") {
            return true;
        }
    ...
    static bool IsMicrosoftBasicType(const opString& s) {
        if (s == "__wchar_t" || s == "__int8" || s == "__int16" ||
            s == "__int32" || s == "__int64") {
            return true;
        }
    ...
    static bool IsSignable(const opString& s) {
        return (s == "char" || s == "short" || s == "long" || s == "int" ||
                s == "__int8" || s == "__int16" || s == "__int32" ||
                s == "__int64");
    }
    ...
    static bool IsIntegerSuffix(const opToken& t) {
        ...
        return (s == "u" || s == "l" || s == "U" || s == "L" || s == "ul" ||
                s == "lu" || s == "UL" || s == "LU" || s == "uL" || s == "Ul" ||
                s == "lU" || s == "Lu" || s == "LL" || s == "ll" ||
                s == "ull" || s == "uLL" || s == "Ull" || s == "ULL" ||
                s == "i8" || s == "I8" || s == "ui8" || s == "uI8" ||
                s == "Ui8" || s == "UI8" || s == "i16" || s == "I16" ||
                s == "ui16" || s == "uI16" || s == "Ui16" || s == "UI16" ||
                s == "i32" || s == "I32" || s == "ui32" || s == "uI32" ||
                s == "Ui32" || s == "UI32" || s == "i64" || s == "I64" ||
                s == "ui64" || s == "uI64" || s == "Ui64" || s == "UI64");
    }
};
...
inline bool BlockMatch(Token left, Token right) {
    if (left == T_LEFT_BRACE && right != T_RIGHT_BRACE)
        return false;
    else if (left == T_LEFT_PAREN && right != T_RIGHT_PAREN)
        return false;
    else if (left == T_LEFT_BRACKET && right != T_RIGHT_BRACKET)
        return false;
...
inline opString RightBlockString(Token left) {
    if (left == T_LEFT_BRACE) return opTokenMap::GetString(T_RIGHT_BRACE);
    if (left == T_LEFT_PAREN) return opTokenMap::GetString(T_RIGHT_PAREN);
    if (left == T_LEFT_BRACKET) return opTokenMap::GetString(T_RIGHT_BRACKET);

    return "";
}

inline opString LeftBlockString(Token right) {
    if (right == T_RIGHT_BRACE) return opTokenMap::GetString(T_LEFT_BRACE);
    if (right == T_RIGHT_PAREN) return opTokenMap::GetString(T_LEFT_PAREN);
    if (right == T_RIGHT_BRACKET) return opTokenMap::GetString(T_LEFT_BRACKET);
...
inline bool IsLeftBlock(Token t) {
    if (t == T_LEFT_BRACE || t == T_LEFT_BRACKET || t == T_LEFT_PAREN)
        return true;
...
inline bool IsRightBlock(Token t) {
    if (t == T_RIGHT_BRACE || t == T_RIGHT_BRACKET || t == T_RIGHT_PAREN)
        return true;
...
inline bool IsBlock(Token t) {
    if (IsLeftBlock(t) || IsRightBlock(t)) return true;
