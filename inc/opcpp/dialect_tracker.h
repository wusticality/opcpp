...
//	x	need to validate all special category note arguments! (should get
//context) 			I should do this by making these DialectNotes :P
//		need to validate expression modifier usage?
//			I guess if I don't do that, is ___ will just cause nothing
//to show up
//			and isnot ____ just wont do anything - it would be good to
//validate this 			how? well, all modifiers can be checked
//from the outer category registration
//				 and all automatic modifiers should be able to be
//listed from maps.
//			I won't add this until later, its just a nice feature -
//since it would
//			only flag issues a single time instead of multiple
//times. 	o	need better errors, with file & line number if
//available.
...
struct DialectSetting {
   public:
    DialectSetting(const opString& name, DialectSetting* parent)
        : Name(name), Parent(parent) {}

    virtual DialectNote* ToNote() { return NULL; }

    virtual DialectMap* ToMap() { return NULL; }

    virtual DialectEnumMap* ToEnumMap() { return NULL; }

    virtual DialectMapExpression* ToMapExpression() { return NULL; }

    virtual DialectLocation* ToLocation() { return NULL; }

    virtual DialectCategory* ToCategory() { return NULL; }
    ...
    DialectMap* GetMap() {
        DialectSetting* current = this;
        while (current != NULL) {
            if (DialectMap* map = current->ToMap()) return map;
            current = current->GetParent();
        }
    ...
    DialectEnumMap* GetEnumMap() {
        DialectSetting* current = this;
        while (current != NULL) {
            if (DialectEnumMap* map = current->ToEnumMap()) return map;
            current = current->GetParent();
        }
    ...
    DialectSetting* GetParent() { return Parent; }
    ...
    opString GetPath() {
        ...
        while (current) {
            ...
            if (current) settingpath = "::" + settingpath;
        }
...
struct DialectNoteBase : DialectSetting {
    ...
    virtual bool GetArgumentDescription(const opString& name,
                                        opString& description);
};
...
struct DialectMapExpression {
    DialectMapExpression(CriteriaNodeBase* expression)
    ...
    CriteriaNodeBase* GetExpression() { return Expression; }
...
struct DialectExpressionMap : public DialectNoteBase {
    DialectExpressionMap(const opString& name, DialectSetting* parent)
...
struct DialectNoParentNote : public DialectNote {
    DialectNoParentNote(const opString& name, DialectSetting* parent,
                        NoteNode* node)
...
struct DialectEnumNote : public DialectNote {
    DialectEnumNote(const opString& name, DialectSetting* parent,
                    NoteNode* node)
        : DialectNote(name, parent, node) {}
...
struct DialectDisallow : public DialectExpressionMap {
    DialectDisallow(const opString& name, DialectSetting* parent,
                    DisallowNode* node)
        : DialectExpressionMap(name, parent), Node(node) {}
    ...
    DisallowNode* GetNode() { return Node; }
...
struct DialectMap : public DialectExpressionMap {
    DialectMap(const opString& name, DialectSetting* setting,
...
struct DialectEnumMap : public DialectNoteBase {
    DialectEnumMap(const opString& name, DialectSetting* parent,
    ...
    EnumerationMapNode* GetMapNode() { return Node; }
...
struct DialectLocationBase : DialectSetting {
    DialectLocationBase(const opString& name, DialectSetting* parent)
        : DialectSetting(name, parent) {}
    ...
    int GetNumOrderedNotes() { return OrderedNotes.Size(); }

    OrderedNoteList& GetOrderedNotes() { return OrderedNotes; }
...
struct DialectLocation : public DialectLocationBase {
    DialectLocation(const opString& name, DialectSetting* parent)
        : DialectLocationBase(name, parent) {}
...
struct DialectEnumLocation : public DialectLocationBase {
    DialectEnumLocation(const opString& name, DialectSetting* parent)
        : DialectLocationBase(name, parent) {}
...
struct DialectCategory : public DialectTypeBase {
   ...
   private:
    ...
    bool RegisterSpecialNote(NoteDefinitionNode* node,
...
struct DialectFileDeclaration {
    ...
    DialectFileDeclaration(const opString& name)
        : Name(name), FilenameNote(NULL) {}

    /*=== utility ===*/

    const opString& GetName() { return Name; }

    NoteDefinitionNode* GetFilenameNote() { return FilenameNote; }
...
class DialectTracker {
   public:
    ...
    static NoteDefinitionNode* GetNote(const opString& category,
   ...
   private:
    ...
    typedef opHashTable<opString, DialectEnumeration*>::iterator
        enumerationiterator;
    ...
    // throws an error if collided, returns true if ok, false if already
    // registered
    ...
    static DialectTracker& GetInstance() { return Instance; }
...
struct TypePath {
    TypePath(DialectTypeBase* type) : Type(type) {}
