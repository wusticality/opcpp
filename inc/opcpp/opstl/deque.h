...
class opDeque {
   public:
    ...
    opDeque(const deque_type& indeque) : d(indeque) {}

    opDeque(const opDeque<T>& indeque) : d(indeque.d) {}

    template <class InputIterator>
    opDeque(InputIterator first, InputIterator last) : d(first, last) {}
    ...
    int MaxSize() const { return (int)d.max_size(); }
    ...
    void PushFront(const T& inval) { d.push_front(inval); }

    void PushBack(const T& inval) { d.push_back(inval); }
    ...
    void Swap(opDeque<T>& indeque) { d.swap(indeque.d); }
    ...
    iterator Erase(int index) { return d.erase(d.begin() + index); }
    ...
    void Resize(size_type num) { d.resize(num); }

    void Add(size_type num = 1) { d.resize(d.size() + num); }
    ...
    iterator Find(const T& inval) { return find(d.begin(), d.end(), inval); }
    ...
    size_type FindIndex(const T& inval) {
        ...
        for (size_type i = 0; i < size; i++) {
            if (d[i] == inval) return i;
        }

        return -1;
    }

    bool Contains(const T& inval) const { return Find(inval) != d.end(); }

    deque_type& GetDeque() { return d; }

    const deque_type& GetDeque() const { return d; }
    ...
    friend bool operator==(const opDeque<T>& indeque1,
                           const opDeque<T>& indeque2) {
   ...
   public:
    ...
    void push_front(const T& inval) { d.push_front(inval); }

    void push_back(const T& inval) { d.push_back(inval); }

    void pop_front() { d.pop_front(); }

    void pop_back() { d.pop_back(); }

    void swap(deque_type& indeque) { d.swap(indeque); }
    ...
    void resize(size_type num, T t = T()) { d.resize(num, t); }
};
