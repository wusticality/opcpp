...
class opSet {
   public:
    ...
    opSet(const set_type& inset) : s(inset) {}

    opSet(const opSet<T>& inset) : s(inset.s) {}

    template <class InputIterator>
    opSet(InputIterator first, InputIterator last) : s(first, last) {}
    ...
    int MaxSize() const { return (int)s.max_size(); }

    bool IsEmpty() const { return s.empty(); }

    void Swap(opSet<T>& inset) { s.swap(inset.s); }
    ...
    void Erase(iterator position) { s.erase(position); }

    void Erase(iterator first, iterator last) { s.erase(first, last); }
    ...
    iterator Find(const T& inval) { return s.find(inval); }

    bool Contains(const T& inval) { return s.find(inval) != s.end(); }

    int Count(const T& inval) { return (int)s.count(inval); }

    bool Includes(const opSet<T>& inset) {
        return includes(s.begin(), s.end(), inset.s.begin(), inset.s.end(),
                        opSetter<T>());
    }

    void Union(const opSet<T>& inset, opSet<T>& result) const {
        result.Clear();
        set_union(s.begin(), s.end(), inset.s.begin(), inset.s.end(),
                  result.s.begin(), opSetter<T>());
    }
    ...
    void Intersection(const opSet<T>& inset, opSet<T>& result) {
        result.Clear();
        set_intersection(s.begin(), s.end(), inset.s.begin(), inset.s.end(),
                         result.s.begin(), opSetter<T>());
    }

    void Difference(const opSet<T>& inset, opSet<T>& result) {
        result.Clear();
        set_difference(s.begin(), s.end(), inset.s.begin(), inset.s.end(),
                       result.s.begin(), opSetter<T>());
    }

    void SymmetricDifference(const opSet<T>& inset, opSet<T>& result) {
        result.Clear();
        set_symmetric_difference(s.begin(), s.end(), inset.s.begin(),
                                 inset.s.end(), result.s.begin(),
                                 opSetter<T>());
    }

    set_type& GetSet() { return s; }

    const set_type& GetSet() const { return s; }
   ...
   public:
    ...
    void swap(const set_type& inset) { s.swap(inset.s); }
    ...
    size_type erase(const key_type& inval) { return s.erase(inval); }

    void erase(iterator first, iterator last) { s.erase(first, last); }

    void clear() { s.clear(); }

    iterator find(const key_type& inval) { return s.find(inval); }

    size_type count(const key_type& inval) { return s.count(inval); }
};
