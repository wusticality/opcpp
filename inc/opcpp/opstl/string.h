...
class opString {
   public:
    typedef basic_string<char, char_traits<char>, OPSTL_STRING_ALLOCATOR(char)>
        string_type;
    ...
    opString(const opString& _s) { s = _s.s; }

    template <class a>
    opString(const basic_string<char, char_traits<char>, a>& _s)
        : s(_s.begin(), _s.end()) {}

    opString(const char* _s) { s = _s; }

    opString(const char c) { s = c; }

    opString(int i) { ToString(i, *this); }

    opString(unsigned int i) { ToString(i, *this); }

    opString(float f) { ToString(f, *this); }

    opString(double d) { ToString(d, *this); }

    opString(bool b) { s = b ? "true" : "false"; }

    // Initialize a string to a line in a file.
    opString(istream& i, const char term = '\n') { Getline(i, term); }

    // Initialize a string given a wide string.
    opString(const wstring& _s) { s = string_type(_s.begin(), _s.end()); }

    opString(const vector<char>& _s) { s = string_type(_s.begin(), _s.end()); }

    opString(const opArray<char>& _s) { s = string_type(_s.begin(), _s.end()); }
    ...
    string_type& GetString() { return s; }

    const string_type& GetString() const { return s; }

    const string_type& get_string() const { return s; }

    string_type& get_string() { return s; }

    const char* GetCString() const { return s.c_str(); }

    char GetCharacter(int i) const { return s[i]; }

    int ToInt() const { return atoi(s.c_str()); }

    float ToFloat() const { return (float)atof(s.c_str()); }

    double ToDouble() const { return (double)atof(s.c_str()); }

    bool ToBool() const { return s == "true" ? true : false; }
    ...
    void Reserve(size_t length) { s.reserve(length); }

    void Resize(size_t length) { s.resize(length); }

    // returns last char
    char Back() const { return s[(int)s.size() - 1]; }
    ...
    opString& Remove(const char c) { return Remove(opString(c)); }
    ...
    opString& TrimLeft() {
        ...
        if (endindex > 0) Remove(0, endindex);
    ...
    opString& TrimRight() {
        ...
        if (endindex + 1 < size) Remove(endindex + 1, size);
    ...
    opString Left(int i) const { return Substring(0, i); }
    ...
    opString Left(const opString& _s) const {
        ...
        if (Find(_s, index)) return Left(index);

        return opString(s);
    }

    // left - char version b/c compiler is an idiot
    opString Left(const char c) const { return Left(opString(c)); }
    ...
    opString RLeft(const opString& _s) const {
        ...
        if (RFind(_s, index)) return Left(index);

        return opString(s);
    }

    opString RLeft(const char c) const { return RLeft(opString(c)); }

    opString RLeft(int i) const { return Left(Length() - i); }
    ...
    opString Right(const opString& _s) const {
        ...
        if (Find(_s, index)) return Right(index + size - 1);

        return opString(s);
    }

    // right - char version b/c compiler should be shot
    opString Right(const char c) const { return Right(opString(c)); }
    ...
    opString RRight(const opString& _s) const {
        ...
        if (RFind(_s, index)) return Right(index + size - 1);

        return opString(s);
    }

    opString RRight(char c) const { return RRight(opString(c)); }

    opString RRight(int i) const { return Right(Length() - i - 1); }
    ...
    bool EndsWith(const opString& other) const {
        ...
        return equal(other.s.begin(), other.s.end(),
                     s.begin() + Length() - olength);
    }
    ...
    // note: searches backward!
    bool RFind(const opString& _ss, int& result,
               int start = (int)string::npos) const {
        string::size_type _start = start;
    ...
    void TokenizeOnWhitespace(T& container) const {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (c == ' ' || c == '\n' || c == '\t') {
                ...
                container.push_back(
                    opString::StringTo<typename T::value_type>(token));
    ...
    static int Hash(const opString& s) { return s.Hash(); }
    ...
    static opString FilledString(char c, int rep) {
        ...
        for (int i = 0; i < rep; i++) newString += c;
    ...
    static bool IsAlphaNumeric(char c) { return IsAlpha(c) || IsNumeric(c); }
    ...
    wstring ToUnicode() const { return std::wstring(s.begin(), s.end()); }
    ...
    // <
    bool operator<(const opString& rhs) const { return s < rhs.s; }

    bool operator<(const string& rhs) const { return s < rhs; }
...
inline opString operator+(const string& left_string,
                          const opString& right_string) {
    return opString(left_string) + right_string;
}

inline opString operator+(const char* left_string,
                          const opString& right_string) {
    return opString(left_string) + right_string;
}
...
struct opHashFunction<opString> {
    static size_t Hash(const opString& t) { return (size_t)t.Hash(); }
};
