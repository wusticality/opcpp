...
class opMap {
   public:
    ...
    opMap(const map_type& inmap) : m(inmap) {}

    opMap(const opMap<Key, Value>& inmap) : m(inmap.m) {}

    template <class InputIterator>
    opMap(InputIterator first, InputIterator last) : m(first, last) {}
    ...
    int MaxSize() const { return (int)m.max_size(); }

    bool IsEmpty() const { return m.empty(); }

    void Swap(opMap<Key, Value>& inmap) { m.swap(inmap.m); }
    ...
    void Erase(iterator position) { m.erase(position); }

    void Erase(const Key& inkey) { m.erase(inkey); }

    void Erase(iterator first, iterator last) { m.erase(first, last); }

    void Clear() { m.clear(); }

    iterator Find(const Key& inkey) { return m.find(inkey); }

    const_iterator Find(const Key& inkey) const { return m.find(inkey); }
    ...
    bool Contains(const Key& inkey) const { return m.find(inkey) != m.end(); }

    int Count(const Key& inkey) const { return (int)m.count(inkey); }

    map_type& GetMap() { return m; }

    const map_type& GetMap() const { return m; }
    ...
    Value& operator[](const Key& inkey) { return m[inkey]; }

    friend bool operator==(const opMap<Key, Value>& m1,
                           const opMap<Key, Value>& m2) {
        return m1.m == m2.m;
    }
   ...
   public:
    ...
    void swap(map_type& inmap) { m.swap(inmap.m); }
    ...
    size_type erase(const key_type& inkey) { return m.erase(inkey); }

    void erase(iterator first, iterator last) { m.erase(first, last); }

    void clear() { m.clear(); }

    iterator find(const key_type& inkey) { return m.find(inkey); }

    const_iterator find(const key_type& inkey) const { return m.find(inkey); }

    size_type count(const key_type& inkey) const { return m.count(inkey); }
};
...
class opMultiMap {
   public:
    ...
    opMultiMap(const map_type& inmap) : m(inmap) {}

    opMultiMap(const opMultiMap<Key, Value>& inmap) : m(inmap.m) {}

    template <class InputIterator>
    opMultiMap(InputIterator first, InputIterator last) : m(first, last) {}
    ...
    map_type& GetMultiMap() { return m; }

    const map_type& GetMultiMap() const { return m; }
    ...
    friend bool operator==(const opMultiMap<Key, Value>& m1,
                           const opMultiMap<Key, Value>& m2) {
        return m1.m == m2.m;
    }
   ...
   public:
    ...
    iterator insert(const value_type& inval) { return m.insert(inval); }
