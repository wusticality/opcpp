...
class opArray {
   public:
    ...
    opArray(const array_type& inarray) : a(inarray) {}

    opArray(const opArray<T>& inarray) : a(inarray.a) {}

    template <class InputIterator>
    opArray(InputIterator first, InputIterator last) : a(first, last) {}
    ...
    const_reverse_iterator RBegin() const { return a.rbegin(); }
    ...
    const_reverse_iterator REnd() const { return a.rend(); }

    int Size() const { return (int)a.size(); }

    int Length() const { return (int)a.size(); }

    int MemSize() const { return Size() * sizeof(T); }

    int MaxSize() const { return (int)a.max_size(); }

    bool IsEmpty() const { return a.empty(); }

    int Capacity() const { return (int)a.capacity(); }

    void Reserve(size_type num) { a.reserve(num); }

    T& Front() { return a.front(); }

    T& Back() { return a.back(); }

    const T& Front() const { return a.front(); }

    const T& Back() const { return a.back(); }

    void PushBack(const T& inval) { a.push_back(inval); }
    ...
    void Swap(opArray<T>& inarray) { a.swap(inarray.a); }
    ...
    iterator Erase(iterator position) { return a.erase(position); }

    iterator Erase(int index) { return a.erase(a.begin() + index); }
    ...
    void Resize(size_type num) { a.resize(num); }

    void Add(size_type num = 1) { a.resize(a.size() + num); }
    ...
    iterator Find(const T& inval) { return find(a.begin(), a.end(), inval); }
    ...
    size_type FindIndex(const T& inval) {
        ...
        for (size_type i = 0; i < size; i++) {
            if (a[i] == inval) return i;
        }

        return -1;
    }

    bool Contains(const T& inval) const { return Find(inval) != a.end(); }
    ...
    friend bool operator==(const opArray<T>& inarray1,
                           const opArray<T>& inarray2) {
   ...
   public:
    ...
    size_type capacity() const { return a.capacity(); }

    void reserve(size_type n) { a.reserve(n); }

    reference front() { return a.front(); }

    reference back() { return a.back(); }

    const_reference front() const { return a.front(); }

    const_reference back() const { return a.back(); }

    void push_back(const T& inval) { a.push_back(inval); }

    void pop_back() { a.pop_back(); }

    void swap(array_type& inarray) { a.swap(inarray); }
    ...
    void resize(size_type num, T t = T()) { a.resize(num, t); }
};
