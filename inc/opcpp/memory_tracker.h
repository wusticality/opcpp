...
struct opHashFunction<opNode*> {
    static size_t Hash(const opNode* const t) { return (size_t)t; }
};
}  // namespace opstl
...
class opMemoryTracker {
   public:
    ...
    //		tracker - they don't need to necessarily have a NEWNODE style
    //wrapper, 		we just want to be made aware of them..
...
class stacked {
   ...
   public:
    ...
    explicit stacked<T>(T* pointer) : node(pointer) {}
   ...
   public:
    ...
    // copy constructor
    stacked<T>(const stacked<T>& pointer) : node(pointer.node) {}

    ~stacked<T>() {
        // int count = opException::GetExceptionCount();
        if (node) {
            if (opException::InException()) delete node;
        }
    }
    ...
    static stacked<T> buildstacked(T* node) { return stacked<T>(node); }
   ...
   private:
    stacked<T>(const stackedcontext<T>& pointer) : node(pointer.GetNode()) {}
};
...
class NewHandler {
   public:
    template <class T>
    static inline stacked<T> NewNode(T* newnode, const opString& file,
                                     int linenumber) {
#ifdef _DEBUG
