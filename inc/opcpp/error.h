...
class opError {
   public:
    ...
    static const opString& GetParseError(Token t) { return ParseErrors[t]; }
   ...
   public:
    ...
    static void RedefinitionError(const opString& name, opNode* olddef,
                                  opNode* newdef);

    // "Member <string message> cannot be same name as enclosing category."
    static void MemberNameSameAsCategoryNameError(const opString& name,
                                                  opNode* category,
                                                  opNode* def);
    ...
    // Expected <type(s)>, before <got> in context <context>, instead got
    // <token>.
    static void ReverseExpectError(const vector<Token>& expected, opNode* got,
                                   opNode* before);
    ...
    // Reached premature end of <location>, Expected <list of grammars>
    static void PrematureError(const vector<Token>& expected, opNode* beginning,
                               bool bEOF);
    ...
    static void InvalidValueError(ValuedModifierNode* modifier,
                                  ModifierNodeBase* definition);

    // Invalid data modifier <Modifier> used, available modifiers: <Valid
    // Modifier List>
    static void InvalidDataModifierError(opNode* badmodifier);

    // Invalid function modifier <Modifier> used, available modifiers: <Valid
    // Modifier List>
    static void InvalidFunctionModifierError(opNode* badmodifier);

    static void ExpandError(opNode* node, const opString& signature,
    ...
    static void NoteDefinitionError(NoteDefinitionNode* oldnode,
                                    NoteDefinitionNode* newnode);

    static void MapRedeclaredError(DialectMap* oldmap, CategoryMapNode* newmap);

    static void NameCollisionError(const opString& name, opNode* oldnode,
                                   opNode* newnode);

    static void DisallowError(DisallowNode* disallow, opNode* location);

    // got a bad operand in a criteria expression
    static void OperandError(opNode* node, DialectExpressionMap* map,
                             const opString& operandname);

    // invalid extendpoint error
    static void InvalidExtensionError(opNode* node, const opString& name,
                                      const opString& type);

    // alt prefix redefinition error
    static void AltPrefixRedefinitionError(const opString& type,
                                           opNode* oldprefix,
                                           opNode* newprefix);

    /*=== generic ===*/

    static void MessageError(opNode* node, const opString& message,
   ...
   private:
    ...
    static void GetDataModifierList(const opString& spacer,
                                    opString& modifierlist, opNode* innernode);
    static void GetFunctionModifierList(const opString& spacer,
                                        opString& modifierlist,
                                        opNode* innernode);
    static void GetList(const opString& spacer, opString& output,
                        opSet<opString>& list);
...
class stackedcontext : public stacked<T>, public AlterContext {
   ...
   public:
    ...
    // 			: stacked<T>(pointer.GetNode()),
    // AlterContext(pointer.GetNode())
    // 		{}
