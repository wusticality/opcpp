...
class OPTypeBase : public ConditionalSupport<ModifierSupport<opNode> > {
   public:
    ...
    void PrintNote(DialectNote& notenode, opSectionStream& stream,
                   NoteType type = Note, int map_count = 0);
    void PrintNote(DialectLocationBase& location, DialectNote& noteitem,
                   opSectionStream& stream);

    // argument fetch functions
    virtual opNode* GetContextArgument(const opString& argumentname);
    opNode* GetModifierArgument(const opString& argumentname,
                                vector<ValuedModifierNode*>& modifiervalues);

    opNode* GetStartArgument(const opString& argumentname,
                             stackedgroup& tempmodifiers);
    opNode* GetMappingArgument(const opString& argumentname, int map_index,
                               stackedgroup& tempmodifiers);
    opNode* GetEndArgument(const opString& argumentname, int map_count,
                           stackedgroup& tempmodifiers);
...
class OPEnumNode : public OPTypeBase {
   public:
    ...
    void SetBody(OPEnumBodyNode* inbody) { Body = inbody; }

    void SetIdentifier(TerminalNode* identifier) { Identifier = identifier; }

    TerminalNode* GetIdentifier() { return Identifier; }

    TerminalNode* GetName() { return Name; }

    OPEnumBodyNode* GetBody() { return Body; }

    opString ErrorName();

    DialectTypeBase* GetTypeSettings() { return GetEnumSettings(); }
   ...
   private:
    // note printing
    void PrintLocationNotes(DialectEnumLocation* settings,
                            opSectionStream& stream);

    void PrintEnumMap(opSectionStream& stream, DialectEnumeration& enumeration,
                      DialectEnumLocation& location, DialectEnumMap& mapinfo);
    ...
    void PrintEnumMapping(opSectionStream& stream,
                          DialectEnumeration& enumeration,
                          DialectEnumLocation& location,
                          DialectEnumMap& mapinfo, OPEnumElementNode* element,
                          int map_index);
    ...
    opNode* GetElementArgument(const opString& argumentname,
...
class OPEnumBodyNode : public OPEnumBodyNodeParent {
   public:
    ...
    void AddElement(OPEnumElementNode* elt) { Elements.push_back(elt); }

    int GetNumElements() { return (int)Elements.size(); }

    OPEnumElementNode* GetLastElement() { return Elements.back(); }
...
class OPEnumInitializationNode : public opNode {
   public:
    ...
    void CloneNode(OPEnumInitializationNode* node) { CloneChildren(node); }
...
class OPEnumElementNode : public OPEnumElementNodeParent {
   public:
    ...
    OPEnumInitializationNode* GetInitialization() { return Initialization; }
...
class OPObjectNode : public OPObjectNodeParent {
   public:
    ...
    void SetName(TerminalNode* t) { Name = t; }

    CPlusPlusNode* GetMiddleModifiers() { return MiddleModifiers; }

    void SetMiddleModifiers(CPlusPlusNode* innode) { MiddleModifiers = innode; }

    OPObjectInheritanceNode* GetInheritance() { return Inheritance; }

    void SetInheritance(OPObjectInheritanceNode* i) { Inheritance = i; }

    void SetBody(OPObjectBodyNode* body) { Body = body; }

    OPObjectBodyNode* GetBody() { return Body; }

    void SetCategory(opNode* category);

    void SetCategoryName(const opString& s) { CategoryName = s; }

    const opString& GetCategoryName() { return CategoryName; }
    ...
    opString GetClassName() { return Name->GetValue(); }
    ...
    opString GetScopedClassName() {
        opString scope = GetClassScope();
        if (scope.Size()) return scope + "::" + GetClassName();
        return GetClassName();
    }
    ...
    DialectTypeBase* GetTypeSettings() { return GetCategorySettings(); }
    ...
    /**** printing ****/
   public:
    void PrintDataMapNote(opSectionStream& stream, const opString& macrotype,
   ...
   private:
    ...
    void PrintMap(DialectLocation& location, DialectMap& mapitem,
                  opSectionStream& stream);

    /**** print data notes ****/
    void PrintDataMap(opSectionStream& stream, DialectCategory& category,
                      DialectLocation& location, DialectMap& mapinfo);

    void PrintDataMapStart(opSectionStream& stream, DialectCategory& category,
                           DialectLocation& location, DialectMap& mapinfo);

    void PrintDataMapping(opSectionStream& stream, DialectCategory& category,
                          DialectLocation& location, DialectMap& mapinfo,
                          DataStatementBase* statement, int map_index);

    void PrintDataMapEnd(opSectionStream& stream, DialectCategory& category,
                         DialectLocation& location, DialectMap& mapinfo,
                         int map_count);

    /**** print function notes ****/
    void PrintFunctionMap(opSectionStream& stream, DialectCategory& category,
                          DialectLocation& location, DialectMap& mapinfo);
    ...
    void PrintFunctionMapping(opSectionStream& stream,
                              DialectCategory& category,
                              DialectLocation& location, DialectMap& mapinfo,
                              FunctionStatementBase* statement, int map_index);

    void PrintFunctionMapEnd(opSectionStream& stream, DialectCategory& category,
                             DialectLocation& location, DialectMap& mapinfo,
                             int map_count);

    /**** argument fetch functions ****/
    opNode* GetContextArgument(const opString& argumentname);
    opNode* GetStatementArgument(const opString& argumentname,
    ...
    // location query
    bool IsInsideClass() { return IsInside(G_OPOBJECT); }
...
class OPObjectInheritanceNode : public OPInheritanceNodeParent {
   public:
    DECLARE_NODE(OPObjectInheritanceNode, OPInheritanceNodeParent,
    ...
    void SetObject(OPObjectNode* node) { Object = node; }

    const opArray<Parent>& GetParents() { return Parents; }
...
class OPObjectBodyNode : public OPObjectBodyNodeParent {
   public:
    ...
    virtual opString GetClassName() { return Object->GetClassName(); }
...
class StateNode : public opNode {
   public:
    ...
    void SetBody(StateBlockNode* inbody) { Body = inbody; }

    StateBlockNode* GetBody() { return Body; }
...
class OPDefineNode : public DialectNodeBase {
   public:
    ...
    void SetArguments(ParenBlockNode* inarguments) { Arguments = inarguments; }

    void SetBody(OPDefineBodyNode* inbody) { Body = inbody; }

    TerminalNode* GetName() { return Name; }

    ParenBlockNode* GetArguments() { return Arguments; }
...
class OPIncludeNode : public DialectNodeBase {
   public:
    ...
    void SetFileName(TerminalNode* filename) { FileName = filename; }
    ...
    FileNode* GetIncludedFile() { return IncludedFile; }
...
class OPMacroArgumentNode : public opNode {
   public:
    ...
    void Init() { VariableName = NULL; }

    bool PreParse();

    opNode* GetVariableName() { return VariableName; }

    opString ErrorName() { return ""; }

    void PrintString(opString& s) { VariableName->PrintString(s); }
...
class OPMacroArgumentListNode : public OPMacroArgumentListNodeParent {
   public:
    DECLARE_NODE(OPMacroArgumentListNode, OPMacroArgumentListNodeParent,
                 G_OPMACRO_ARGUMENT_LIST);

    bool PreParse();

    const vector<OPMacroArgumentNode*>& GetArguments() { return Arguments; }
...
class OPMacroNode : public DialectNodeBase {
   public:
    ...
    void SetBody(OPMacroBodyNode* inbody) { Body = inbody; }

    TerminalNode* GetName() { return Name; }

    OPMacroArgumentListNode* GetArguments() { return Arguments; }

    OPMacroBodyNode* GetBody() { return Body; }
   ...
   private:
    void ExpandChildren(opNode* cloned, ExpandCallArgumentNode* replacement,
...
class ValuedModifierArgumentNode : public opNode {
   public:
    DECLARE_NODE(ValuedModifierArgumentNode, opNode,
    ...
    void PrintString(opString& s) { PrintStringUnspacedChildren(s); }
...
class ValuedModifierNode : public opNode {
   public:
    ...
    void SetModifier(TerminalNode* node) { Modifier = node; }

    void SetArguments(ValuedModifierArgumentNode* node) { Arguments = node; }

    TerminalNode* GetModifier() { return Modifier; }

    ValuedModifierArgumentNode* GetArguments() { return Arguments; }
...
class CPlusPlusNode : public opNode {
   public:
    ...
    void SetBody(CPlusPlusBodyNode* body) { Body = body; }
