...
#define DECLARE_NODE_BASE(typ, supr, grammar, file, linenum)           \
   public:                                                             \
    ...
    static opString StaticGetNodeTypeSource() {                        \
        return GetNodeTypeFile() + "(" + GetNodeTypeLine() + ")";      \
    }                                                                  \
    opString GetNodeTypeSource() { return StaticGetNodeTypeSource(); } \
    const char* GetNodeType() { return #typ; }                         \
    stacked<typ> Clone() { return CloneProcess<typ>(this); }           \
...
#define DECLARE_NODE(typ, supr, grammar) \
    DECLARE_NODE_BASE(typ, supr, grammar, __FILE__, __LINE__)
...
#define PREPARSE_START                            \
    AlterContext newcontext(this->GetId(), this); \
    if (!Super::PreParse()) return false;         \
    this->ResetPosition();                        \
    INIT_EXCEPTIONS
...
#define POSTPARSE_START                           \
    AlterContext newcontext(this->GetId(), this); \
    if (!Super::PostParse()) return false;        \
    this->ResetPosition();                        \
    INIT_EXCEPTIONS
...
struct endlinestruct {
    ...
    friend ostream& operator<<(ostream& o,
                               const workarounds::endlinestruct& endline) {
        return o << '\n';
    }
};
}  // namespace workarounds
...
#ifdef _DEBUG
#define DebugAllocationNumber(number) \
    if (opNode::GetStaticAllocationNumber() == number) opPlatform::Breakpoint();
#endif
...
#define DECLARE_VISIT(nodetype)                         \
    virtual bool enter(nodetype* node) { return true; } \
