...
class TerminalNode : public opNode {
   public:
    ...
    void CloneNode(TerminalNode* newnode) { newnode->value = value; }
    ...
    TerminalNode(const opString& invalue, Token intoken, int line,
                 FileNode* infile) {
        value = invalue;
        SetId(intoken);
        SetLine(line);
    ...
    void PrintValue(opFileStream& s) { PrintValue(s.header.body); }

    void PrintValue(opSectionStream& s) { PrintValue(s.body); }

    // PrintNode : FileStream version (multi-file)
    void PrintNode(opFileStream& s) { PrintValue(s.header.body); }

    // PrintNode : FileSection version (single-file)
    void PrintDialectNode(opDialectStream& s) { PrintValue(s.header.heading); }

    void PrintOriginal(opSectionStream& s) { PrintValue(s); }

    void PrintTransformed(opSectionStream& s) { PrintValue(s); }

    void PrintString(opString& s) { s += value; }
...
class NamespaceNode : public opNode {
   public:
    ...
    void SetBody(NamespaceBlockNode* inbody) { Body = inbody; }
...
class NamespaceAliasNode : public opNode {
   public:
    ...
    void AddScope(TerminalNode* value) { Scopes.PushBack(value); }
...
class NamespaceBlockNode : public NamespaceBlockNodeParent {
   public:
    DECLARE_NODE(NamespaceBlockNode, NamespaceBlockNodeParent,
                 G_NAMESPACEBLOCK);
...
class UsingNode : public UsingNodeParent {
   public:
    ...
    void SetScope(ScopeNode* inscope) { Scope = inscope; }
...
class PointerNode : public opNode {
   public:
    ...
    void SetType(opNode* intype) { Type = intype; }

    opNode* GetType() { return Type; }

    void AddStar(StarType type) { Stars.PushBack(type); }

    StarType GetStarType(int index) { return Stars[index]; }

    int GetNumStars() { return Stars.Size(); }
    ...
    void CloneNode(PointerNode* node) {
        ...
        int num = GetNumStars();
        for (int i = 0; i < num; i++) node->AddStar(Stars[i]);
    }
...
typedef DataStatementOnly<FunctionArgumentDefinitionBase>
    FunctionArgumentDefinitionNodeParent;

class FunctionArgumentDefinitionNode
    : public FunctionArgumentDefinitionNodeParent {
   public:
    DECLARE_NODE(FunctionArgumentDefinitionNode,
                 FunctionArgumentDefinitionNodeParent,
...
class FunctionArgumentDefaultNode : public opNode {
   public:
    DECLARE_NODE(FunctionArgumentDefaultNode, opNode,
...
class FunctionArgumentNode : public opNode {
   public:
    ...
    FunctionArgumentDefinitionNode* GetDefinition() { return Definition; }

    FunctionArgumentDefaultNode* GetDefault() { return Default; }

    void SetDefinition(FunctionArgumentDefinitionNode* node) {
        Definition = node;
    }

    void SetDefault(FunctionArgumentDefaultNode* node) { Default = node; }
    ...
    void CloneNode(FunctionArgumentNode* node) {
        ...
        if (Default) {
            stacked<FunctionArgumentDefaultNode> clonedefault =
                Default->Clone();
...
class FunctionArgumentListNode : public context::Argument<opNode> {
   public:
    DECLARE_NODE(FunctionArgumentListNode, context::Argument<opNode>,
    ...
    void AddArgument(FunctionArgumentNode* node) { Arguments.push_back(node); }

    int GetNumArguments() { return (int)Arguments.size(); }

    FunctionArgumentNode* GetArgument(int index) { return Arguments[index]; }
...
class FunctionNode : public opNode {
   public:
    ...
    FunctionArgumentListNode* GetArguments() { return Arguments; }

    TerminalNode* GetAssignment() { return Assignment; }

    bool IsConst() { return bConst; }

    void SetName(opNode* inName) { Name = inName; }

    void SetArguments(FunctionArgumentListNode* inArguments) {
        Arguments = inArguments;
    }

    void SetConst(bool bconst) { bConst = bconst; }

    void SetAssignment(TerminalNode* node) { Assignment = node; }
...
class FunctionPrototypeNode : public opNode {
   public:
    ...
    opNode* GetReturn() { return Return; }

    FunctionNode* GetFunction() { return Function; }

    void SetReturn(opNode* inReturn) { Return = inReturn; }

    void SetFunction(FunctionNode* inFunction) { Function = inFunction; }
...
class FunctionDefinitionNode : public opNode {
   public:
    ...
    void SetBody(BraceBlockNode* inBody) { Body = inBody; }
...
class DestructorNode : public opNode {
   public:
    ...
    void Init() { function = NULL; }

    opNode* GetName() { return function->GetName(); }

    void SetFunction(FunctionNode* node) { function = node; }
...
class DestructorPrototypeNode : public opNode {
   public:
    ...
    void SetDestructor(DestructorNode* node) { Destructor = node; }

    DestructorNode* GetDestructor() { return Destructor; }
...
class ConstructorPrototypeNode : public opNode {
   public:
    ...
    void SetConstructor(ConstructorNode* node) { Constructor = node; }

    ConstructorNode* GetConstructor() { return Constructor; }

    void PrintOriginal(opSectionStream& stream);

    opString ErrorName();

    void PrintString(opString& s) { return Constructor->PrintString(s); }
...
class ConstructorInitializerListNode : public opNode {
   public:
    DECLARE_NODE(ConstructorInitializerListNode, opNode,
...
class ConstructorInitializerArgumentNode : public opNode {
   public:
    DECLARE_NODE(ConstructorInitializerArgumentNode, opNode,
...
class ConstructorDefinitionNode : public opNode {
   public:
    ...
    ConstructorInitializerListNode* GetInitializers() { return Initializers; }
...
class TemplateTypeBodyNode : public TemplateTypeBodyNodeParent {
   public:
    DECLARE_NODE(TemplateTypeBodyNode, TemplateTypeBodyNodeParent,
                 G_TEMPLATE_TYPE_BODY);

    void Clone(TemplateTypeBodyNode* node) { CloneChildren(node); }
    ...
    void PrintString(opString& s) { Super::PrintString(s); }
...
class TemplateTypeNode : public opNode {
   public:
    ...
    void SetBody(TemplateTypeBodyNode* innode) { Braces = innode; }

    TemplateTypeBodyNode* GetBody() { return Braces; }
...
class TemplateDeclNode : public opNode {
   public:
    ...
    void Init() { braces = NULL; }

    void SetBraces(AngledBlockNode* inbraces) { braces = inbraces; }

    void PrintTransformed(opSectionStream& stream);

    void PrintOriginal(opSectionStream& stream) { PrintTransformed(stream); }
...
class ArrayNode : public opNode {
   public:
    ...
    void SetName(TerminalNode* n) { Name = n; }

    TerminalNode* GetName() { return Name; }

    void AddBracket(BracketBlockNode* b) { Brackets.push_back(b); }

    int GetNumBrackets() { return (int)Brackets.size(); }

    BracketBlockNode* GetBracket(int index) { return Brackets[index]; }
...
class VisibilityLabelNode : public opNode {
   public:
    ...
    VisibilityMode GetVisibility() { return Visibility; }
...
class ScopeNode : public opNode {
   public:
    ...
    void Init() { bGlobal = false; }

    // adds a scope to Scope vector
    //
    // example: hello::there <==> Scope {hello,there}
    void AddScope(opNode* inscope) { Scope.push_back(inscope); }

    // put a new scope on the front of the existing scope
    void PrependScope(opNode* inscope) { Scope.push_front(inscope); }
    ...
    opNode* GetScopeNode(int index) { return Scope[index]; }

    int GetScopeNum() { return (int)Scope.size(); }

    void SetGlobal(bool newglobal) { bGlobal = newglobal; }
...
class ScopePointerNode : public opNode {
   public:
    ...
    void SetNumStars(int stars) { Stars = stars; }
...
class PointerMemberNode : public opNode {
   public:
    ...
    void SetType(opNode* node) { Type = node; }

    void SetScope(ScopePointerNode* node) { Scope = node; }
...
class TypedefNode : public opNode {
   public:
    ...
    void SetModifier(TerminalNode* innode) { Modifier = innode; }

    void SetType(opNode* innode) { Type = innode; }

    void SetName(TerminalNode* innode) { Name = innode; }

    /*=== get ===*/

    TerminalNode* GetModifier() { return Modifier; }
...
class FunctionPointerDeclNode : public FunctionPointerDeclNodeParent {
   public:
    DECLARE_NODE(FunctionPointerDeclNode, FunctionPointerDeclNodeParent,
    ...
    opNode* GetScope() { return Scope; }

    void SetName(opNode* node) { Name = node; }

    void SetScope(opNode* node) { Scope = node; }
...
class FunctionPointerNode : public opNode {
   public:
    ...
    void SetReturn(opNode* ret) { Return = ret; }

    opNode* GetReturn() { return Return; }

    void SetDecl(FunctionPointerDeclNode* decl) { Decl = decl; }

    FunctionPointerDeclNode* GetDecl() { return Decl; }

    void SetArguments(opNode* args) { Arguments = args; }

    opNode* GetArguments() { return Arguments; }

    opNode* GetName() { return Decl->GetName(); }
...
class FundamentalTypeNode : public opNode {
   public:
    ...
    void SetType(TerminalNode* innode) { Type = innode; }

    void SetIsSigned(bool bSigned) { Signed = bSigned; }
...
class FriendNode : public opNode {
   public:
    ...
    enum ForwardDeclType { Class, Struct, None };
    ...
    void SetFriend(opNode* innode) { Friend = innode; }

    opNode* GetFriend() { return Friend; }

    void SetSemicolon(bool inbool) { bHasSemicolon = inbool; }

    bool HasSemicolon() const { return bHasSemicolon; }

    void SetForwardDecl(opNode* inforward) { ForwardDecl = inforward; }
...
class CPPConstructInheritanceNode : public opNode {
   public:
    DECLARE_NODE(CPPConstructInheritanceNode, opNode,
...
class CPPConstructObjectNode : public CPPConstructNode {
   public:
    ...
    void Init() { Inheritance = NULL; }

    void SetInheritance(CPPConstructInheritanceNode* innode) {
        Inheritance = innode;
    }

    CPPConstructInheritanceNode* GetInheritance() { return Inheritance; }
...
class EnumNode : public CPPConstructNode {
   public:
    ...
    opString ErrorName() { return Super::ErrorName(); }
};
...
class TemplatedNode : public opNode {
   public:
    ...
    void SetTheTemplate(TemplateDeclNode* node) { TheTemplate = node; }

    TemplateDeclNode* GetTheTemplate() { return TheTemplate; }

    void SetTemplated(opNode* node) { Templated = node; }

    opNode* GetTemplated() { return Templated; }
