...
opString FunctionDefinitionStatementNode::GetSignatureString() {
    ...
    if (GetModifiers()) GetModifiers()->PrintBuiltInReturnArgument(signature);
    ...
    GetFunctionDefinition()->GetFunction()->GetArguments()->PrintSignature(
        signature);
...
void FuncPrototypeStatementNode::PrintMacroArguments(opSectionStream& stream) {
    FunctionArgumentListNode* args =
        FunctionPrototype->GetFunction()->GetArguments();
...
void OPObjectNode::PrintVisibility(VisibilityMode mode,
                                   opSectionStream& stream) {
    if (mode == vismode_none) return;
    ...
    if (mode == vismode_public) {
        ...
        stream.PrintComment("//default note: " + category->GetName() +
                            "::" + note->GetName());
        stream << endl;
