...
void FillSectionStreams(opSectionStream& section, const opString& type,
                        const opString& basemacro, const opString& bigname,
                        const opString& smallname) {
    ...
    if (!opParameters::Get().Compact)
        section.footerstring +=
            opString("\t// == ") + smallname + " section end == //\n";
};
...
void OPFileNode::PrintNode(opFileStream& stream) {
    ...
    FillSectionStreams(stream.header.heading, "OOH", hmacro, "HEADING",
                       "heading");
    ...
    FillSectionStreams(stream.source.heading, "OCPP", smacro, "HEADING",
                       "heading");
    FillSectionStreams(stream.source.body, "OCPP", smacro, "BODY", "body");
    FillSectionStreams(stream.source.footer, "OCPP", smacro, "FOOTER",
                       "footer");
}
...
void OPFileNode::PrintXml(opXmlStream& stream) {
    ...
    opString filedir =
        "file://" +
        path((opPlatform::GetOpCppDirectory() + "/../../../Css/").GetString())
            .string() +
        "/";
...
void DialectFileNode::PrintHighlighting(opSectionStream& stream) {
    ...
    for (int i = start; i < end; i++) {
        ...
        if (name != "@") HighlightKeyword(stream, name);
    }
    ...
    for (int i = start; i < end; i++) {
        ...
        if (name != "c++") HighlightKeyword(stream, name);
    }
...
void FunctionNode::PrintOriginal(opSectionStream& stream) {
    ...
    if (bConst) stream << " const";

    if (Assignment) stream << " = " << Assignment;
}
...
void FunctionNode::PrintString(opString& s) {
    ...
    if (Assignment) s += " = " + Assignment->GetValue();
}
...
void FunctionArgumentListNode::PrintNameOnly(opSectionStream& stream) {
    ...
    int size = (int)Arguments.size();
    for (int i = 0; i < size; i++) {
        DataStatementNode* datastatement =
            Arguments[i]->GetDefinition()->GetInnerStatement();
...
void FunctionArgumentListNode::PrintSignature(opString& s) {
    ...
    for (int i = 0; i < size; i++) {
        ModifiersNode* mods =
            Arguments[i]->GetDefinition()->GetInnerStatement()->GetModifiers();
        if (mods) mods->PrintBuiltInArgument(s);
...
void FunctionArgumentNode::PrintOriginal(opSectionStream& stream) {
    if (Definition) Definition->PrintOriginal(stream);
...
void FunctionArgumentNode::PrintString(opString& s) {
    if (Definition) Definition->PrintString(s);
...
void OPEnumNode::PrintNode(opFileStream& stream) {
    ...
    stream.header.body.PrintComment(
        "//special note: " + enumsettings->GetName() + "::identifier");
    ...
    stream.header.body.PrintComment(
        "//special note: " + enumsettings->GetName() + "::parent");
    ...
    // Print Body Location
    DialectEnumLocation* bodylocationsettings =
        enumsettings->FindLocation(DialectLocation::body);
    ...
    // Print Post Location
    DialectEnumLocation* postlocationsettings =
        enumsettings->FindLocation(DialectLocation::post);
    PrintLocationNotes(postlocationsettings, stream.header.body);

    // Print Header Location
    //	if(!IsInside(G_OPOBJECT))
    {
        DialectEnumLocation* headinglocationsettings =
            enumsettings->FindLocation(DialectLocation::heading);
        PrintLocationNotes(headinglocationsettings, stream.header.heading);
    }

    // Print Footer Location
    DialectEnumLocation* footerlocationsettings =
        enumsettings->FindLocation(DialectLocation::footer);
    PrintLocationNotes(footerlocationsettings, stream.header.footer);

    // Print Source Location
    DialectEnumLocation* sourcelocationsettings =
        enumsettings->FindLocation(DialectLocation::source);
    PrintLocationNotes(sourcelocationsettings, stream.source.body);

    // Print SourceHeading Location
    DialectEnumLocation* sourceheadinglocationsettings =
        enumsettings->FindLocation(DialectLocation::sourceheading);
    PrintLocationNotes(sourceheadinglocationsettings, stream.source.heading);

    // Print SourceFooter Location
    DialectEnumLocation* sourcefooterlocationsettings =
        enumsettings->FindLocation(DialectLocation::sourcefooter);
...
opString OPObjectNode::GetParentName() {
    ...
    if (Inheritance) Inheritance->PrintParentNames(str);
...
void OPObjectNode::PrintInheritance(opSectionStream& stream) {
    ...
    bool bprintparent = parentnote.GetNoteDefinition()
                            ? !parentnote.GetNoteDefinition()->IsBodyEmpty()
                            : false;
    bool bprintnoparent = noparentnote.GetNoteDefinition()
                              ? !noparentnote.GetNoteDefinition()->IsBodyEmpty()
                              : false;

    if (bprintnoparent) bprintparent = false;
...
void OPObjectNode::PrintNode(opFileStream& stream) {
    if (opParameters::Get().Notations || opParameters::Get().PrintXml) {
        FetchAllModifiers();
    ...
    stream.header.body.PrintComment(
        "//special note: " + categorysettings->GetName() + "::identifier");
    PrintNote(identifiernote, stream.header.body);
    ...
    // print header maps
    DialectLocation* headinglocationsettings =
        categorysettings->FindLocation(DialectLocation::heading);
    PrintLocationNotes(headinglocationsettings, stream.header.heading);

    // print footer maps
    DialectLocation* footerlocationsettings =
        categorysettings->FindLocation(DialectLocation::footer);
    PrintLocationNotes(footerlocationsettings, stream.header.footer);

    // print source maps
    DialectLocation* sourceheadinglocationsettings =
        categorysettings->FindLocation(DialectLocation::sourceheading);
    PrintLocationNotes(sourceheadinglocationsettings, stream.source.heading);

    // print source maps
    DialectLocation* sourcelocationsettings =
        categorysettings->FindLocation(DialectLocation::source);
    PrintLocationNotes(sourcelocationsettings, stream.source.body);

    // print source maps
    DialectLocation* sourcefooterlocationsettings =
        categorysettings->FindLocation(DialectLocation::sourcefooter);
    PrintLocationNotes(sourcefooterlocationsettings, stream.source.footer);
    ...
    DialectLocation* bodylocationsettings =
        categorysettings->FindLocation(DialectLocation::body);
    PrintLocationNotes(bodylocationsettings, stream.header.body);
    ...
    DialectLocation* postlocationsettings =
        categorysettings->FindLocation(DialectLocation::post);
    PrintLocationNotes(postlocationsettings, stream.header.body);
...
bool DataStatementBase::GetModifierDescription(const opString& name,
    ...
    if (name.EndsWith("()")) {
        ...
        if (DataModifierNode* node =
                GetCategorySettings()->FindValueDataModifier(name.RLeft(2))) {
        ...
        if (DataModifierNode* node =
                GetCategorySettings()->FindDataModifier(name)) {
            if (node->GetDescription(description)) return true;
        }
    }
...
bool FunctionStatementBase::GetModifierDescription(const opString& name,
                                                   opString& description) {
    if (name.EndsWith("()")) {
        if (ModifierDescriptions::GetValuedModifier(name, description))
            return true;
        ...
        if (FunctionModifierNode* node =
                GetCategorySettings()->FindValueFunctionModifier(
                    name.RLeft(2))) {
            if (node->GetDescription(description)) return true;
        }
    } else {
        ...
        if (FunctionModifierNode* node =
                GetCategorySettings()->FindFunctionModifier(name)) {
            if (node->GetDescription(description)) return true;
        }
    }
        ...
        if (mods) {
            ...
            while (it != end) {
...

                {
                    ...
                    if (basic) stream << basic->GetValue();
                    if (valued) stream << valued->GetModifier()->GetValue();
                }
                ...
                opString modname;
                if (basic) modname = basic->GetValue();
                if (valued) modname = valued->GetModifier()->GetValue() + "()";
...
void OPObjectBodyNode::PrintTransformedBody(opFileStream& stream) {
    ...
    while (i != end) {
        if (StatementBase* statement = i->ToStatementBase()) {
            if (VisibilityStatementNode* vis =
                    statement->ToVisibilityStatementNode()) {
                ...
                if (!bFoundFirst) {
                    if (mode == vismode_none) mode = vismode_default;

                    Object->PrintVisibility(mode, stream.header.body);

                    lastprinted = mode;
                    bFoundFirst = true;
                } else if (mode == vismode_none && lastprinted != lastlabel) {
                    Object->PrintVisibility(lastlabel, stream.header.body);

                    lastprinted = lastlabel;
                } else if (mode != vismode_none && mode != lastprinted) {
                    Object->PrintVisibility(mode, stream.header.body);
                    lastprinted = mode;
                }
            }
...
void ArrayNode::PrintString(opString& s) {
    ...
    for (int i = 0; i < (int)Brackets.size(); i++) Brackets[i]->PrintString(s);
}
...
void FriendNode::PrintOriginal(opSectionStream& stream) {
    ...
    if (bHasSemicolon) stream << ';';
}

FriendNode::ForwardDeclType FriendNode::GetForwardDeclType() {
    if (!ForwardDecl) return None;
    if (ForwardDecl->GetId() == T_STRUCT) return Struct;
    if (ForwardDecl->GetId() == T_CLASS) return Class;
    return None;
}
...
void OPDefineNode::PrintTransformed(opSectionStream& stream) {
    ...
    if (Arguments) Arguments->MacroPrint(stream, numchar);
...
void OPIncludeNode::PrintNode(opFileStream& stream) {
    ...
    stream.header.heading << "#include \""
                          << opDriver::ToGeneratedPath(GetFullFileString())
                          << ".ooh"
                          << "\"" << endl;
    stream.header.body << "#include \""
                       << opDriver::ToGeneratedPath(GetFullFileString())
                       << ".ooh"
                       << "\"" << endl;
    stream.header.footer << "#include \""
                         << opDriver::ToGeneratedPath(GetFullFileString())
                         << ".ooh"
                         << "\"" << endl;
    stream.source.heading << "#include \""
                          << opDriver::ToGeneratedPath(GetFullFileString())
                          << ".ooh"
                          << "\"" << endl;
    stream.source.body << "#include \""
                       << opDriver::ToGeneratedPath(GetFullFileString())
                       << ".ooh"
                       << "\"" << endl;
    stream.source.footer << "#include \""
                         << opDriver::ToGeneratedPath(GetFullFileString())
                         << ".ooh"
                         << "\"" << endl;
...
void OPIncludeNode::PrintOriginal(opSectionStream& stream) {
    ...
    if (IsInside(G_CODE)) IncludedFile->PrintOriginal(stream);
}
...
void DataStatementNode::PrintTransformed(opSectionStream& stream) {
    if (modifiers) modifiers->PrintBuiltIn(stream);

    Declaration->PrintTransformed(stream);

    // if we have static we need to print initialization, otherwise don't
    // (hardcoded transform!)
    if (Initialization) {
...
void ModifiersNode::PrintBuiltIn(opSectionStream& stream) {
    ...
    while (i != end) {
        ...
        if (token == T_STATIC || token == T_CONST || token == T_INLINE ||
            token == T_VOLATILE || token == T_VIRTUAL || token == T_EXPLICIT ||
            token == G_CPLUSPLUS) {
            i->PrintOriginal(stream);
            stream << ' ';
        }
...
void ModifiersNode::PrintBuiltInReturnArgument(opString& s) {
    ...
    while (i != end) {
        ...
        if (token == T_CONST || token == T_VOLATILE) {
            i->PrintString(s);
            s += ' ';
        }
...
void ModifiersNode::PrintBuiltInArgument(opString& s) {
    ...
    while (i != end) {
        ...
        if (token == T_CONST || token == T_VOLATILE)  // is volatile allowed?
        {
            i->PrintString(s);
            s += ' ';
        }
...
void ModifiersNode::PrintBuiltInSource(opSectionStream& stream) {
    ...
    while (i != end) {
        ...
        if (token == T_CONST || token == T_EXPLICIT) {
            i->PrintOriginal(stream);
            stream << ' ';
        }
...
void StateStatementNode::PrintNode(opFileStream& stream) {
    // 	vector<FunctionDefinitionStatementNode*>& FunctionStatements =
...
void ConstructorDefinitionStatementNode::PrintPrototype(
...
void ConstructorDefinitionStatementNode::PrintScopedDefinition(
    opSectionStream& stream) {
    ...
    stream.PrintComment(
        "//moved constructor definition: " + scope +
    ...
    if (modifiers) modifiers->PrintBuiltInSource(stream);
...
void ConstructorDefinitionStatementNode::PrintOriginal(
    opSectionStream& stream) {
...
void ConstructorDefinitionNode::PrintOriginal(opSectionStream& stream) {
    ...
    if (Initializers) Initializers->PrintOriginal(stream);
...
void ConstructorNode::PrintString(opString& s) { function->PrintString(s); }
//
// DestructorNode
//
...
void DestructorDefinitionStatementNode::PrintPrototype(
    opSectionStream& stream) {
    // print built in modifiers
    if (modifiers) modifiers->PrintBuiltIn(stream);
...
void DestructorDefinitionStatementNode::PrintScopedDefinition(
    opSectionStream& stream) {
    OPObjectNode* object = FindParent<OPObjectNode>();
    DialectNote& prefixnote = object->GetCategorySettings()->GetPrefixNote();
    DialectNote& postfixnote = object->GetCategorySettings()->GetPostfixNote();
    ...
    stream.PrintComment(
        "//moved destructor definition: " + scope +
...
void FunctionDefinitionStatementNode::PrintTransformed(
    opSectionStream& stream) {
    if (modifiers) modifiers->PrintBuiltIn(stream);
...
void FunctionDefinitionStatementNode::PrintFunction(opFileStream& stream) {
    ...
    ValuedModifierNode* namespace_start = node_cast<ValuedModifierNode>(
        parent->GetContextArgument("namespace_start"));
    ValuedModifierNode* namespace_end = node_cast<ValuedModifierNode>(
        parent->GetContextArgument("namespace_end"));
    ...
    if (bInline) {
        if (bPrintNamespaces) {
            namespace_start->GetArguments()->PrintOriginal(
                stream.header.footer);
            stream.header.footer << endl;
        }

        // TODO: we only need class_scope, dont need to print the full namespace
        // scope  now write the scoped signature & body to the header footer
...
void FunctionDefinitionStatementNode::PrintScopedDefinition(
    opSectionStream& stream) {
    OPObjectNode* object = FindParent<OPObjectNode>();
    DialectNote& prefixnote = object->GetCategorySettings()->GetPrefixNote();
    DialectNote& postfixnote = object->GetCategorySettings()->GetPostfixNote();
    ...
    stream.PrintComment(
        "//moved function definition: " + scope +
...
void FuncPointerStatementNode::PrintOriginal(opSectionStream& stream) {
    if (modifiers) modifiers->PrintOriginal(stream);
...
void CategoryMapNode::PrintDialectNode(opDialectStream& stream) {
    ...
    if (Body) Body->PrintDialectNode(stream);
...
void NoteArgumentNode::PrintString(opString& s) { PrintStringChildren(s); }
...
void DataModifierNode::PrintString(opString& s) {
    ...
    if (Arguments) Arguments->PrintString(s);
}
...
void CriteriaArgumentNode::PrintString(opString& s) {
    s += "(";
    if (Argument) s += Argument->GetValue();
    s += ")";
}
...
int PreprocessorStatementNode::PrintCondition(opSectionStream& stream) {
    ...
    if (Previous != NULL) num = Previous->PrintCondition(stream);
    ...
    if (node_cast<PoundIfNode>(node) || node_cast<PoundIfndefNode>(node) ||
