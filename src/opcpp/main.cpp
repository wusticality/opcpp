...
#ifdef MALLOC_DEBUGGING
    // create a memory checkpoint - because we don't want string literals or
    // static initializations to show up  we only want to check things allocated
...
// use _CrtSetBreakAlloc(number listed in report);
    ...
    if (!opPlatform::ParseExeLocation(argv[0])) return -1;
    ...
    for (int i = 1; i < argc; i++) params.push_back(opString(argv[i]));
    ...
    if (!opDemoSupport::ValidateLicense(p)) {
        if (!p.Version) opVersion::PrintVersion();
    }
