...
bool FileNode::Preprocessor() {
    INIT_EXCEPTIONS {
        ...
        if (!CheckBlockCorrectness()) return false;
...
bool FileNode::CheckBlockCorrectness() {
    ...
    while (i != end) {
        ...
        if (IsLeftBlock(Id)) lefts.PushFront(*i);
        // right block, verify it
            ...
            // make sure the top left and this new right block match
                ...
                opError::MessageError(
                    lefts.Front(),
                    error + frontvalue + "' without closing '" +
    ...
    opError::MessageError(lefts.Front(),
                          error + frontvalue + "' without terminating '" +
                              RightBlockString(lefts.Front()->GetId()) + "'",
                          false);
...
bool AngledBlockNode::Parse() {
    ...
    // were this a known template angled bracket, we could check for other
    // things as well.
...
bool FunctionPointerDeclNode::Parse() {
    ...
    // in GCC attributes are always of format __attribute__((att1, att2,
    // att3(args)))  theres also synonyms for each like __noreturn__
...
bool FunctionArgumentListNode::Parse() {
    ...
    if (!IsEmpty()) {
        while (1) {
            stacked<FunctionArgumentNode> arg =
                opNode::PushUntil<FunctionArgumentNode>(T_COMMA, false);

            if (arg->IsEmpty()) arg->CopyBasics(this);
...
bool FunctionArgumentNode::Parse() {
    ...
    stacked<FunctionArgumentDefinitionNode> definition =
        opNode::PushUntil<FunctionArgumentDefinitionNode>(T_ASSIGN, false);

    Definition = *definition;
    InsertNodeAtCurrent(definition);

    if (IsCurrent(T_ASSIGN)) Erase(T_ASSIGN);

    stacked<FunctionArgumentDefaultNode> initialization =
        opNode::PushUntilEnd<FunctionArgumentDefaultNode>();
...
bool OPEnumBodyNode::Parse() {
    ...
    while (IsCurrent(T_ID)) {
        stacked<OPEnumElementNode> newNode =
            opNode::PushUntil<OPEnumElementNode>(T_COMMA, false);
        newNode->Init();

        if (IsCurrent(T_COMMA)) Erase(T_COMMA);
...
bool OPEnumElementNode::Parse() {
    ...
    if (IsCurrent(T_ASSIGN)) {
        ...
        stacked<OPEnumInitializationNode> init =
            PushUntilEnd<OPEnumInitializationNode>();
...
bool OPObjectInheritanceNode::Parse() {
    ...
    if (IsCurrent(T_COLON)) {
        ...
        while (1) {
            ...
            if (IsCurrent(T_PUBLIC) || IsCurrent(T_PROTECTED) ||
                IsCurrent(T_PRIVATE)) {
                p.Visibility =
                    CheckOr<TerminalNode>(T_PUBLIC, T_PROTECTED, T_PRIVATE);
            }
    ...
    PARSE_START;
    {
        LOOP_START(G_CONSTRUCTOR_INITIALIZER) {
            HIT(G_FUNCTION) {
                ...
                // other types, then reconstruct the type (kinda weird, but it
                // works)
                stacked<FunctionNode> f =
                    opNode::Expect<FunctionNode>(G_FUNCTION);
                stacked<FunctionArgumentListNode> args =
                    f->PopNode<FunctionArgumentListNode>(f->GetArguments());
                stacked<opNode> name = f->PopNode<opNode>(f->GetName());

                stacked<ConstructorInitializerArgumentNode> cargs =
                    opNode::Transform<ConstructorInitializerArgumentNode>(args);
                stacked<ConstructorInitializerNode> constructor =
                    opNode::Transform<ConstructorInitializerNode>(f);
...
bool DataDeclarationNode::Parse() {
    PARSE_START;
    {
        ...
        if (IgnoreUntil<T_COLON>()) {
            ...
            stackedcontext<DataBitsNode> bits =
                opNode::Make<DataBitsNode>(T_COLON);
            Erase(T_COLON);

            stacked<TerminalNode> number =
                opNode::Expect<TerminalNode>(T_NUMBER);
...
void DataDeclarationNode::ParseTypeName() {
    ...
    stacked<opNode> type = opNode::ReverseExpectOr(
        T_ID, G_POINTER, G_REFERENCE, G_TEMPLATE_TYPE, G_SCOPE, T_BASIC_TYPE,
        G_FUNDAMENTAL_TYPE, G_POINTER_MEMBER, *name);
    Type = *type;
...
bool CriteriaBodyNode::Parse() {
    PARSE_START;
    {
        ...
        while (1) {
            opNode* operand =
                CheckOr(T_ID, G_CRITERIA_VALUE_MODIFIER, G_CRITERIA_GROUP,
                        T_PUBLIC, T_PRIVATE, T_PROTECTED, T_VOLATILE, T_STATIC,
                        T_EXPLICIT, T_CONST, T_VIRTUAL, T_MUTABLE, T_INLINE);
            AddOperand(operand);

            // has an operator?
            if (IsCurrent(T_E_AND) || IsCurrent(T_E_OR)) {
