...
bool OPObjectNode::PostParse() {
    ...
    POSTPARSE_START;
    {
        ...
        if (!ProcessDisallows()) return false;
...
void OPObjectNode::ProcessVisibility() {
    ...
    for (iterator it = Body->GetBegin(); it != end; ++it) {
        ...
        if (VisibilityStatementNode* statement =
                node->ToVisibilityStatementNode()) {
...
void OPObjectNode::RegisterAutoModifiers() {
    ...
    // ModifierDelegate
    // alt_class_name(this,&OPObjectNode::ModifierAltClassName);
    ...
    // ModifierDelegate
    // category_statement(this,&OPObjectNode::ModifierCategoryStatement);
    ...
    if (HasPureFunctions()) {
        // ModifierDelegate
        // category_abstract(this,&OPObjectNode::ModifierCategoryAbstract);
    ...
    if (!GetInheritance()) {
        // ModifierDelegate
        // category_baseclass(this,&OPObjectNode::ModifierCategoryBaseclass);
    ...
    if (GetInheritance()) {
        if (GetInheritance()->GetParents().Size() == 1) {
            // ModifierDelegate
            // category_single_inheritance(this,&OPObjectNode::ModifierCategorySingleInheritance);
            // RegisterModifier("category_single_inheritance",category_single_inheritance);
            RegisterModifier(
                "category_single_inheritance",
                boost::bind(&OPObjectNode::ModifierCategorySingleInheritance,
                            this, _1));
        }

        // ModifierDelegate
        // category_multiple_inheritance(this,&OPObjectNode::ModifierCategoryMultipleInheritance);
        // RegisterModifier("category_multiple_inheritance",category_multiple_inheritance);
        RegisterModifier(
            "category_multiple_inheritance",
            boost::bind(&OPObjectNode::ModifierCategoryMultipleInheritance,
                        this, _1));
    }

    if (HasConstructor()) {
        // ModifierDelegate
        // category_has_constructor(this,&OPObjectNode::ModifierCategoryHasConstructor);
        // RegisterModifier("category_has_constructor",category_has_constructor);
        RegisterModifier(
            "category_has_constructor",
            boost::bind(&OPObjectNode::ModifierCategoryHasConstructor, this,
                        _1));
    }

    if (HasDestructor()) {
        // ModifierDelegate
        // category_has_destructor(this,&OPObjectNode::ModifierCategoryHasDestructor);
        // RegisterModifier("category_has_destructor",category_has_destructor);
        RegisterModifier(
            "category_has_destructor",
            boost::bind(&OPObjectNode::ModifierCategoryHasDestructor, this,
                        _1));
    }

    if (HasSimpleConstructor()) {
        // ModifierDelegate
        // category_has_simple_constructor(this,&OPObjectNode::ModifierCategoryHasSimpleConstructor);
        // RegisterModifier("category_has_simple_constructor",category_has_simple_constructor);
        RegisterModifier(
            "category_has_simple_constructor",
            boost::bind(&OPObjectNode::ModifierCategoryHasSimpleConstructor,
                        this, _1));
    }

    if (HasComplexConstructor()) {
        // ModifierDelegate
        // category_has_complex_constructor(this,&OPObjectNode::ModifierCategoryHasComplexConstructor);
        // RegisterModifier("category_has_complex_constructor",category_has_complex_constructor);
        RegisterModifier(
            "category_has_complex_constructor",
            boost::bind(&OPObjectNode::ModifierCategoryHasComplexConstructor,
                        this, _1));
    }
}
...
void OPObjectNode::ProcessStatements() {
    ...
    // if we errored, throw?
    if (!bresult) opException::ThrowException();
}
...
bool OPObjectNode::ProcessDisallows() {
    ...
    if (!ProcessObjectDisallows()) return false;
    ...
    for (iterator it = Body->GetBegin(); it != end; ++it) {
        ...
        if (!node->ProcessDisallows(CategorySettings)) bErrored = true;
    }

    return !bErrored;
}

bool FunctionStatementBase::ProcessDisallows(
...
bool OPObjectNode::HasConstructor() {
    ...
    while (it != end) {
        ...
        // does the function have
        opNode* node = *it;
        if (ConstructorStatementBase* statement =
...
bool OPObjectNode::HasDestructor() {
    ...
    while (it != end) {
        ...
        // does the function have
        opNode* node = *it;
        if (DestructorStatementBase* statement =
    ...
    while (it != end) {
        ...
        if (ConstructorStatementBase* statement =
            ...
            if (ConstructorNode* ctor = statement->GetConstructor()) {
                if (ctor->GetFunction()->GetArguments()->GetNumArguments() ==
                    0) {
                    return true;
                }
            }
