...
bool opStringStream::bLineDirectives =
    true;  // true - should be true, just for testing!
...
FileNode* FileNode::GetLoadedFile(const opString& filename) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        if (node->GetInputName() == filename) return node;
    }
...
FileNode::~FileNode() {
    if (GetParent()) {
        ...
        if (item != end) *item = NULL;
    }
}

void FileNode::UnRegisterLoadedFiles() {
    for (int i = 0; i < FileTable.Size(); i++) {
        if (FileTable[i]) FileTable[i]->UnRegisterReachable();
    }
}

void FileNode::DeleteLoadedFiles() {
    for (int i = 0; i < FileTable.Size(); i++) {
        if (FileTable[i]) delete FileTable[i];
    }
...
path FindIncludedFile(const path& filepath) {
    ...
    for (dequeit it = p.Directories.begin(); it != dend; ++it) {
        ...
        if (exists(combined)) return combined;
    }
...
FileNode* OPIncludeNode::IncludeFile() {
    ...
    // NOTE: I don't think this process is very memory safe, should utilize
    // stacked more.

    // include and parse based on inclusion context
    if (IsInside(G_CODE)) {
        // code node (including cpp source)
        IncludedFile = FileNode::Load<CppFileNode>(
            actualfile, opScanner::SM_BasicMode, true);
    } else if (IsInside(G_OPFILE)) {
        IncludedFile = FileNode::Load<OPFileNode>(
            actualfile, opScanner::SM_NormalMode, true);
    } else if (IsInside(G_DIALECTFILE)) {
        IncludedFile = FileNode::Load<DialectFileNode>(
            actualfile, opScanner::SM_DialectMode, true);
    } else
        assert(0);
    ...
    //		I dont think we should be tracking 'types' of errors in here though
    //(or in filenode even)

    // 	validpath = filepath;
    //
    // 	//pre-pend generated path, append .ooh
    // 	generatedpath = path(opParameters::Get().GeneratedDirectory.GetString())
    // / (validpath.string() + ".ooh");
    //
    // 	//nice error printout
    // 	if(IncludedFile->AnyErrors())
    // 	{
    // 		//TODO: this should really be handled by post-message contexts set by
    // objects 		if(IncludedFile->FileNotFoundError())
    // 			opError::MessageError(this,opString("opinclude file not found (\"") +
    // filestring + "\")", false); 		else
    // if(IncludedFile->ScanError())
    // 			opError::MessageError(this,"opinclude file scanner error",
    // false); 		else if(IncludedFile->ParseError())
    // 			opError::MessageError(this,"opinclude file parser error",
    // false); 		else if(IncludedFile->LoopError())
    // 			opError::MessageError(this, "opinclude file parser find error",
    // false);
    //
...
void FileNode::PrintIncluded(opXmlStream& stream) { PrintXmlChildren(stream); }
...
bool FileNode::LoadDependencies(const opString& filepath) {
    ...
    if (ifs.is_open()) {
        ...
        for (int i = 0; i < size; i++) Dependencies.Insert(files[i].Trim());
...
bool FileNode::IsDependencyNewer(time_t timestamp) {
    ...
    while (it != end) {
        ...
        if (exists(dependpath)) {
            ...
            if (dependtime > timestamp) return true;
        } else
            return true;
...
bool FileNode::PreOperations() {
    OPERATIONS_START {
        ...
        if (expandnodes.Errored()) return false;
