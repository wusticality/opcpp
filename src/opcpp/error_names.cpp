...
opString OPObjectBodyNode::ErrorName() {
    if (!Object || !Object->GetCategorySettings()) return "";
...
opString ArrayNode::ErrorName() {
    ...
    for (int i = 0; i < num; i++) Brackets[i]->PrintString(name);
...
opString FunctionNode::ErrorName() {
    ...
    opString name;
    // Name->PrintString(name);//NOTE: this is potentially sketchy for
    // operators. use errorname instead?  Better?
    opString err = Name->ErrorName();
    if (!err.Length()) err = opError::GetParseError(Name->GetId());
...
opString DestructorDefinitionStatementNode::ErrorName() {
    ...
    if (DestructorDefinition) fname = DestructorDefinition->ErrorName();
...
opString DestructorPrototypeStatementNode::ErrorName() {
    ...
    if (Destructor) fname = Destructor->ErrorName();
...
opString ConstructorInitializerListNode::ErrorName() {
    if (ConstructorDefinitionNode* parent =
            FindParent<ConstructorDefinitionNode>()) {
        if (!parent->GetConstructor()) return "";
...
opString ConstructorInitializerArgumentNode::ErrorName() {
    if (ConstructorInitializerNode* parent =
            FindParent<ConstructorInitializerNode>()) {
        if (!parent->GetName()) return "";
...
opString CPlusPlusNode::ErrorName() { return ""; }
...
opString CodeNode::ErrorName() { return ""; }

opString CodeBodyNode::ErrorName() { return ""; }
...
opString FriendStatementNode::ErrorName() {
    if (Friend) return Friend->ErrorName();
...
opString UsingStatementNode::ErrorName() {
    if (Using) return Using->ErrorName();
...
opString TypedefStatementNode::ErrorName() {
    if (Typedef) return Typedef->ErrorName();
...
opString FuncPointerStatementNode::ErrorName() {
    ...
    if (FunctionPointer) fname = FunctionPointer->ErrorName();
...
opString FunctionDefinitionStatementNode::ErrorName() {
    ...
    if (FunctionDefinition) fname = FunctionDefinition->ErrorName();
...
opString FuncPrototypeStatementNode::ErrorName() {
    ...
    if (FunctionPrototype) fname = FunctionPrototype->ErrorName();
...
opString ConstructorDefinitionStatementNode::ErrorName() {
    ...
    if (ConstructorDefinition) fname = ConstructorDefinition->ErrorName();
...
opString ConstructorPrototypeStatementNode::ErrorName() {
    ...
    if (Constructor) fname = Constructor->ErrorName();
...
opString OPObjectStatementNode::ErrorName() {
    ...
    if (Object) fname = Object->ErrorName();
...
opString DataModifierNode::ErrorName() {
    ...
    if (Arguments) Arguments->PrintString(name);
...
opString CriteriaArgumentNode::ErrorName() {
    ...
    if (CriteriaValueModifierNode* parent =
            node_cast<CriteriaValueModifierNode>(GetParent())) {
        if (!parent->GetName()) return "";
...
opString ModifierArgumentNode::ErrorName() {
    if (DataModifierNode* parent = node_cast<DataModifierNode>(GetParent())) {
        ...
        return name;
    } else if (FunctionModifierNode* parent =
                   node_cast<FunctionModifierNode>(GetParent())) {
        opString name;
        parent->GetName()->PrintString(name);
        name += "(" LCHEV " \"optional regex expression\" " RCHEV ")";
        return name;
...
opString CategoryMapBodyNode::ErrorName() {
    if (CategoryDatamapNode* parent =
            node_cast<CategoryDatamapNode>(GetParent())) {
        opString name;
        name = "datamap ";
        if (parent->GetName())
            parent->GetName()->PrintString(name);
        ...
        name += " {" LCHEV " ... " RCHEV "}";
        return name;
    } else if (CategoryFunctionmapNode* parent =
                   node_cast<CategoryFunctionmapNode>(GetParent())) {
        opString name;
        name = "functionmap ";
        if (parent->GetName())
            parent->GetName()->PrintString(name);
...
opString EnumerationLocationBodyNode::ErrorName() {
    if (EnumerationLocationNode* parent =
            FindParent<EnumerationLocationNode>()) {
        if (!parent->GetName()) return "";
