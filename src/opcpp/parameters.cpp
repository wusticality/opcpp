...
// constructor
opOption::opOption(const opString& name, const opString& description,
                   bool hidden)
    : Name(name), Description(description), bHidden(hidden), bUsed(false) {
    // register this
...
// constructor
opBoolOption::opBoolOption(const opString& name, const opString& description,
                           bool hidden, bool defaultvalue)
...
opListOption::opListOption(const opString& name, const opString& description,
                           bool hidden)
    : opOption(name, description, hidden) {}
...
opStringOption::opStringOption(const opString& name,
                               const opString& description, bool hidden,
                               const opString& defaultvalue)
...
opIntOption::opIntOption(const opString& name, const opString& description,
                         bool hidden, int defaultvalue)
    : opOption(name, description, hidden), Value(defaultvalue) {}
...
opParameters& opParameters::GetWritable() {
    if (Instance == NULL) Instance = new opParameters();
...
opParameters::opParameters()
    ...
    :
      ...
      // Version
      Version("version",
              "Prints the opC++ compiler version to the standard out.", false),

      // License
      License("license",
              "Location of the compiler's license file.  The default is the "
              "directory in which the opC++"
              "\n\tcompiler executable resides.",
              false,
              opPlatform::GetOpCppDirectory().MakeValidPath() +
                  opDemoSupport::LicenseFileName),

      // Files
      Files("oh", "Input code (.oh) files to be compiled, separated by commas.",
            false),

      // Directories
      Directories("d",
                  "Include directories for both code (.oh) and dialect (.doh) "
                  "files, separated by commas.",
                  false),

      // FileDirectories
      FileDirectories("ohd",
                      "List of directories separated by commas.  All .oh files "
                      "within these directories will be compiled.",
                      false),

      // Depend
      Depend("dependencies",
             "List of additional dependencies separated by commas.  Files will "
             "be tracked by the opC++ compiler and detected changes will cause "
             "a full rebuild.",
             false),
      ...
      // GlobMode
      GlobMode("globmode",
               "Enables glob indexing mode.  In glob mode, the compiler "
               "creates index source and header files"
               "\n\t(Generated.oohindex, Generated.ocppindex) that include all "
               "generated code files.  In this way,"
               "\n\tyou can include all your generated code in a project by "
               "including 'Generated.oohindex'."),

      // CleanMode
      CleanMode("clean",
                "Removes all generated files (*.ooh, *.ocpp, "
                "Generated.oohindex, Generated.ocppindex)."),

      // Verbose
      Verbose("verbose", "Tells the compiler to print verbose output."),

      // Diagnostics
      Diagnostics("diagnostics",
                  "Tells the compiler to print diagnostic output.  This "
                  "includes dialect validation messages."),

      // Silent
      Silent("silent", "The compiler will only print out compiler errors."),

      // Force
      Force("force", "Forces the compiler to recompile everything."),

      // NoDebug
      NoDebug("nodebug",
              "Suppress #line directives (suppress debugging redirection "
              "support)."),
      ...
      // Ghosts
      Ghosts("ghosts",
             "Tells the compiler to output ghost namespaces to improve IDE "
             "auto-completion."),

      // Highlighting
      Highlighting("highlighting",
                   "Tells the compiler to generate highlighting #define's "
                   "based on your dialect(s)."
                   "\n\tNote: This only works if you have a good syntax "
                   "highlighting engine installed (e.g., Visual Assist)."),

      // Notations
      Notations("notations",
                "The compiler will place note expansion comments in the "
                "generated code."),

      // Compact
      Compact("compact",
              "Almost all comments and empty lines are removed from the "
              "generated code."),

      // PrintXml
      PrintXml("printxml",
               "The compiler will generate an xml representation of your opC++ "
               "code."),
      ...
      OPMacroExpansionDepth("expansion-depth",
                            "Specifies the maximum opmacro expansion depth.",
                            false, 100),

      // FixedSys
      FixedSys("fixedsys", "Fixes problems with fixedsys font output."),

      /*=== debug options (these options are hidden) ===*/

      // PrintTree (hidden)
      PrintTree("tree", "Prints the abstract syntax tree to the standard out.",
                true),
      ...
      DeveloperMode("developer",
                    "Puts the compiler into developer mode.  This will force "
                    "the compiler to print information specifically for "
                    "developers.",
                    true),
...
void opParameters::Init(const vector<opString>& params) { Params = params; }
...
opString opParameters::GetCommandLineString() const {
    ...
    for (size_t i = 0; i < Params.size(); i++) Args += Params[i] + ' ';
...
opOption* opParameters::GetOption(const opString& optionname) {
    ...
    for (int i = 0; i < num; i++)
        if (Options[i]->GetName() == optionname) return Options[i];
...
bool opParameters::Parse() {
    ...
    if (Files.GetUsed()) NormalMode = true;

    // use of "-ohd" implies NormalMode = true
    if (FileDirectories.GetUsed()) NormalMode = true;
...
bool opParameters::Validate() {
    ...
    if (!NormalMode && GlobMode && PrintTree) {
        Log("Warning: Print tree ignored - can only print tree when compiling "
            "files.");
        Log("");
...
void opParameters::PrintSyntax() {
    ...
    Syntax +=
        "      Example:  opcpp -oh file1.oh,\"file 2.oh\" -d dir1,dir2 "
        "-verbose";
    ...
    for (int i = 0; i < num; i++) {
        ...
        if (!bHidden || (bHidden && bPrintHidden)) {
            Log("");
...
bool opParameters::SetParameter(const opString& optionname) {
    ...
    if (option->HasArgument()) return false;

    if (Verbose) Log("setparameter " + optionname);
...
bool opParameters::SetParameter(const opString& optionname,
    ...
    if (Verbose) Log("setparameter " + optionname + " " + value);
