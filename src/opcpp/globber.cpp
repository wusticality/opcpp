...
//			path a lot more (its slow converting between them too
//probably).
...
bool Globber::Glob(const opParameters& p) {
    ...
    //			if the matching ooh and ocpp files exist (because maybe its not in
    //the project anymore
    //				...orphaned or whatever)
    //		and we only want to update the indexes if they're out of date (if no
    //ooh files changed why update it?) 			so, only if the
    //above conditions hold do we include an oh file in the index and print the
    //index.
    ...
    //		the issue is .doh file output, it looks like an oh file (maybe use
    //.h?)
            ...
            if (ocppstring == oohstring) {
                ...
                if (exists(ohpath)) {
                    if (opString(ohpath.leaf().c_str()).Right('.') == extension)
                        validohfiles.push_back(
                            ohfileinfo(ohpath, oohpath, ocpppath));
                }
            }
        }
    }
    ...
    //    - past this, we want sectioned streams (which aren't required, but
    //    enable other later things)

    // now we have all the valid filenames!
    if (validohfiles.size() && p.Verbose) {
        for (size_t i = 0; i < validohfiles.size(); i++)
            Log(opString("Globber: found oh file : ") +
                validohfiles[i].ohfilepath.string());
    }

    // now we need to check whether to build any of the indexes..

    // 1.
    // determine whether or not to update the ooh index (only if there is an ooh
    // newer than an oh)  so, we need to iterate over all the ooh paths  and see if
    // any of the files are newer than this..
...
void Globber::UpdateIndex(const vector<ohfileinfo>& files,
                          const path& indexpath, const opParameters& p) {
    bool bForce = p.Force;

    if (!exists(indexpath)) bForce = true;
    ...
    if (!bForce) {
        ...
        for (size_t i = 0; i < files.size(); i++) {
            ...
            // the header condition is not so important - that just says that an
            // oh file wasn't build (oops)
            if (indextime < opcpptime) {
                if (p.Verbose)
                    Log(opPlatform::GetOpCppExecutableName() +
                        " newer than index, forcing glob");
    ...
    if (bNewer || bForce) {
        if (p.Verbose)
            Log(string("Globber: ") + indexname +
                " out of date, rebuilding...");
...
template <bool bheader>
void Globber::WriteIndex(const vector<ohfileinfo>& files, const path& indexpath,
                         const opParameters& p) {
    ...
    if (o.is_open()) {
        ...
        //		in linux and windows file systems, but which are not valid
        //in 		the preprocessor
        ...
        // how this works is... sections with NOSECTION defined will not be
        // compiled  but theres no similar requirement (SECTION define) for
        // compiling a section...
...
template <bool bheader>
void Globber::WriteIndexList(FileWriteStream& o,
                             const vector<ohfileinfo>& files,
                             const opString& macroname) {
    o << endl;
...
template <bool bheader>
void Globber::WriteDialectList(FileWriteStream& o,
                               const vector<opString>& dialects,
                               const opString& macroname) {
    o << endl;
...
void Globber::FindFilesInDirectory(const opString& directory,
                                   const opString& extension,
                                   vector<opString>& foundfiles) {
