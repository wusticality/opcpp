...
bool opNode::PreProcess() {
    if (PreParse()) return PreProcessChildren();
...
bool opNode::PostProcess() {
    if (PostParse()) return PostProcessChildren();
...
// pre parse
bool opNode::PreParse() { return true; }
...
bool opNode::PostParse() {
    ...
    /*POSTPARSE_START;
    {
            ...
            if (TokenFunctions::IsOPCPPKeyword(id) ||
    TokenFunctions::IsTemporaryGrammar(id))
            {
...
bool ExpandCallNode::Expand(opSymbolTracker& tracker, opNode::iterator expandit,
                            opNode* parent) {
    ...
    if (OPMacroNode* macro = tracker.OPMacroRegistered(signature)) {
        ...
        if (ExpansionDepth >
            opParameters::Get().OPMacroExpansionDepth.GetValue()) {
            opString error =
        ...
        if (Arguments) {
            ...
            if (expander.Errored()) opException::ThrowException();
        }
        ...
        // A. expand within the expand argument (this makes some sort of sense)
        // - I think this is best.  how?  1. find expands within argument  2. run
        // pre-process on this early
...
bool OPMacroNode::PreProcess() {
    ...
    if (Arguments && !Arguments->PreProcess()) return false;
