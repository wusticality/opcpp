...
// print a mapping
void OPObjectNode::PrintMap(DialectLocation& location, DialectMap& mapitem,
                            opSectionStream& stream) {
...
void OPTypeBase::PrintNote(DialectNote& note, opSectionStream& stream,
                           NoteType type, int map_count) {
    ...
    for (int i = 0; i < numargs; i++) {
        ...
        if (!argumentvalue && type == End)
            argumentvalue =
                GetEndArgument(argumentname, map_count, tempmodifiers);

        if (!argumentvalue) {
            // error couldn't find argument
            // this should never happen in release
            // since it should be caught earlier
            // opError::MessageError(this,"internal error! couldn't find note
            // argument: " + argumentnames[i]);
            Log("internal error! couldn't find note argument: " +
                argumentnames[i]);
...
opNode* OPEnumNode::GetElementArgument(const opString& argumentname,
...
opNode* OPTypeBase::GetModifierArgument(
    const opString& argumentname, vector<ValuedModifierNode*>& modifiervalues) {
...
opNode* OPTypeBase::GetStartArgument(const opString& argumentname,
                                     stackedgroup& tempmodifiers) {
    return NULL;
}

opNode* OPTypeBase::GetMappingArgument(const opString& argumentname,
                                       int map_index,
                                       stackedgroup& tempmodifiers) {
...
void OPEnumNode::PrintEnumMap(opSectionStream& stream,
    ...
    while (i != end) {
        ...
        if (element) {
            ...
            PrintEnumMapping(stream, enumeration, location, mapinfo, element,
                             map_index);
            ++map_index;
        }
...
void OPEnumNode::PrintEnumMapping(opSectionStream& stream,
                                  ...
                                  DialectEnumLocation& location,
                                  DialectEnumMap& mapinfo,
                                  OPEnumElementNode* element, int map_index) {
    const opParameters& p = opParameters::Get();

    DialectNote& note = element->GetInitialization() ? mapinfo.GetValueNote()
                                                     : mapinfo.GetNameNote();
    NoteDefinitionNode& notenode = *note.GetNoteDefinition();
    ...
    for (int i = 0; i < numargs; i++) {
        ...
        // 		if(!argumentvalue)
        // 			argumentvalue = GetModifierArgument(argumentname,
        // arguments);
        if (!argumentvalue)
        ...
        if (!argumentvalue) {
            ...
            // this should never happen in release
            // since it should be caught earlier
            Log("internal error! couldn't find enum mapping argument: " +
                argumentnames[i]);
            opException::ThrowException();
        } else {
            if (p.Notations) {
...
void OPObjectNode::PrintDataMap(opSectionStream& stream,
    ...
    while (i != end) {
        ...
        if (statement) {
            ...
            PrintDataMapping(stream, category, location, mapinfo, statement,
                             map_index);
            ++map_index;
        }
...
void OPObjectNode::PrintDataMapping(opSectionStream& stream,
    ...
    for (int i = 0; i < numargs; i++) {
        ...
        if (!argumentvalue) {
            ...
            // this should never happen in release
            // since it should be caught earlier
            Log("internal error! couldn't find data mapping argument: " +
                argumentnames[i]);
            opException::ThrowException();
        } else {
            if (p.Notations) {
